...
unsigned int get_number_of_rules(FILE *fp) {
  ...
  while (fgets(line, MAXLINELEN, fp) != NULL) {
    if ((strncmp(line, "RULE", 4) == 0) || (strncmp(line, "BSRULE", 4) == 0))
      counter++;
  }
  return counter;
}
...
int send_command_to_kernel(const command *cmd) {
  ...
  int bytes_sent = 0;
  if ((bytes_sent = send_to_kernel((void *)cmd, nh_control, CONTROL_DATA)) <
      0) {
...
int wait_acknowledgement(void) {
  ...
  if (read_command_from_kernel(&cmdack) < 0) {
    PRED,
        printf(TR("wait_acknowledgement(): error receiving ack from kernel!"));
    PNL;
    return -1;
  }
...
int send_rules_to_kernel(ipfire_rule *rules, int nrules) {
  ...
  for (i = 0; i < nrules; i++) {
    ...
    if (send_command_to_kernel(&cmd) < 0) {
      printf(RED "send_rules_to_kernel(): error sending rule %d to kernel!" NL,
             i + 1);
      return -1;
    }
...
int set_myaddr_flags(ipfire_rule *r, int direction, int meaning, int hook) {
  ...
  default:
    printf(RED "\"me\" can be set only in INPUT or OUTPUT directions!" NL);
    return -1;
  }
}
...
int parse_rulefile_and_fill(FILE *fp, ipfire_rule *ipfr, int whichfile) {
  ...
  while (fgets(line, MAXLINELEN, fp) != NULL) {
    ...
    while (i < MAXMULTILEN) {
      snprintf(key, MAXLINELEN, (i > 0 ? "SRCADDR%d_NOT=" : "SRCADDR_NOT="),
               i + 1);
      if (strncmp(line, key, strlen(key)) == 0) {
        ...
        if (get_address(&address, line) > 0) {
          ...
          if (i == 0)
            ...
            struct in_addr ina;
            ina.s_addr = arule.ip.ipsrc[i];
            // 		printf("multiple source addresses DIFFERENT FROM:
            // element %d: %s\n", i, inet_ntoa(ina));
          }
        } else
          perror("Error getting source address from line");
    ...
    while (i < MAXMULTILEN) {
      snprintf(key, MAXLINELEN, (i > 0 ? "DSTADDR%d_NOT=" : "DSTADDR_NOT="),
               i + 1);
      if (strncmp(line, key, strlen(key)) == 0) {
        if (get_address(&address, line) > 0) {
          arule.nflags.dst_addr = ONEADDR;
          ...
          if (i == 0)
            ...
            // 		struct in_addr ina;
            // 		ina.s_addr = arule.ip.ipdst[i];
            // 		printf("multiple destination addresses DIFFERENT FROM:
            // element %d: %s\n", i, inet_ntoa(ina));
          }
        } else
          perror("Error getting source address from line");
    ...
    while (i < MAXMULTILEN) {
      snprintf(key, MAXLINELEN, (i > 0 ? "SRCPORT%d_NOT=" : "SRCPORT_NOT="),
               i + 1);
      if (strncmp(line, key, strlen(key)) == 0) {
        sport = htons((u16)get_integer(line));
        /* [...] checks! */
        ...
        if (i == 0)
          ...
          // 	    printf("multiple source ports different from: element %d:
          // %d\n", i, ntohs(sport));
        }
      }
      i++;
    ...
    while (i < MAXMULTILEN) {
      snprintf(key, MAXLINELEN, (i > 0 ? "DSTPORT%d_NOT=" : "DSTPORT_NOT="),
               i + 1);
      if (strncmp(line, key, strlen(key)) == 0) {
        dport = htons((u16)get_integer(line));
        /* [...] checks! */
        ...
        if (i == 0)
          ...
          // 	    printf("multiple destination ports different from: element
          // %d: %d\n", i, ntohs(dport));
        }
      }
      i++;
    ...
    if (((strncmp(line, "RULE", 4) == 0) && (ruleno > 0)) ||
      ...
      if ((strncmp(line, "BSRULE", 6) == 0) && (whichfile == DENIAL))
      ...
      if (arule.direction == NODIRECTION) {
        printf(RED
               "Error: parameter \"DIRECTION=OUTPUT|PRE|POST|INPUT|FORWARD\""
      ...
      //~ {
      //~ printf(RED "Error: parameter PROTOCOL not specified or not correct!"
      // NL); ~ printf(RED "RULE N. %d" NL, ruleno); ~ goto error; ~ }
      ...
      next_policy_is_blacksite = 1; /* 1st rule blacksite */
               ...
               0) /* no more used, but do not signal as an error... */
...
conflicting_parameters:
  printf(RED "You specified a value for a parameter and now you mean\n"
             "the opposite: bad! Check line %d." NL,
         linenum);
  return -1;
}
  ...
  *number_of_rules = parse_rulefile_and_fill(fprules, ipfire_rules, whichfile);
  ...
  if (uops.dns_resolver) {
    if (unlock_sem(semid_lockfile) < 0)
      PRED, printf(TR("Error unlocking semaphore!"));
    PNL;
  }
  ...
  if ((*number_of_rules != numrules) && (numrules > 0)) {
    printf(TR("Error! Number of rules mismatch [%d vs %d]! Strange!"), numrules,
           *number_of_rules);
    PNL;
...
void print_kernel_userspace_exit_status(const command *exc, int rc_enabled) {
  if (exc->cmd == ROOT_NOFLUSHED)
    ;
  //     printf(TR("%d rules flushed: \"-noflush\" or \"-load\" or \"-rc\"
  //     option?"),
  // 	   exc->anumber), PNL;
  else
    printf(TR("%d rules have been removed from firewall."), exc->anumber), PNL;
}
...
int save_rules() {
  ...
  if (uops.dns_resolver) {
    ...
    if (lock_sem(semid_lockfile) < 0) {
      PRED, printf(TR("Error locking semaphore!"));
      PNL;
      return -1;
    }
  }
  ...
  while (whichfile < 3) {
    if (whichfile == DENIAL) {
      strncpy(filename, uops.blacklist_filename, MAXFILENAMELEN);
      rules = denial_rules;
      rnum = den_rules_num;
    } else if (whichfile == ACCEPT) {
      strncpy(filename, uops.permission_filename, MAXFILENAMELEN);
      rules = accept_rules;
      rnum = acc_rules_num;
    } else if (whichfile == TRANSLATION) {
      strncpy(filename, uops.translation_filename, MAXFILENAMELEN);
...
int flush_request(const struct netl_handle *nh_control, int flush_com) {
  command flush_cmd;
  ...
  if (send_to_kernel((void *)&flush_cmd, nh_control, CONTROL_DATA) < 0)
    return -1;
  init_command(&flush_cmd);
  if (read_from_kern(nh_control, (unsigned char *)&flush_cmd, sizeof(command)) <
      0) {
  ...
  if (flag == RELOAD_FILE) {
    ...
    case TRANSLATION:
      if (getuid() == 0) {
        ...
        PRED,
            printf(TR("You are not root: not freeing old translation rules.")),
            PNL;
      break;
    }
  }
...
int ipfire_openlog(const struct userspace_opts *uo) {
  extern FILE *fplog;
  if (uo->clearlog)
  ...
  if (fplog == NULL) {
    PRED, perror(TR("ipfire_openlog(): error opening log file")), PNL;
    return -1;
  }
  return 0;
}
...
int push_rule_at_pos(ipfire_rule *oldv, ipfire_rule *newv, int position,
                     const int nmax, const ipfire_rule *newr) {
  int i, k;
  int index;
  if (position <= 0) {
    printf(RED "push_rule_at_pos(): position must be >= 0!" NL NL);
    return -1;
  }
  ...
  /* last rules */
  for (k = index + 1; i < nmax && k < nmax + 1; i++, k++) {
    newv[k] = oldv[i];
    newv[k].position = k + 1;
  }
  return k;
...
int pop_rule_from_pos(ipfire_rule *oldv, ipfire_rule *newv, int position,
                      const int nmax) {
  int index, i;
...
int add_rule_at_position(const ipfire_rule *r, int position) {
  ...
  case ACCEPT:
    ...
    if (push_rule_at_pos(accept_rules, tmp_v_rules, position, acc_rules_num,
                         r) < 0)
      return -1;
    acc_rules_num++;
    /* free old vector */
  ...
  case DENIAL:
    ...
    if (push_rule_at_pos(denial_rules, tmp_v_rules, position, den_rules_num,
                         r) < 0)
      return -1;
    den_rules_num++;
    /* free old vector */
...
int delete_rule_at_position(int position, int which_vector) {
  ...
  case ACCEPT:
    ...
    v_tmp = (ipfire_rule *)malloc(sizeof(ipfire_rule) * (acc_rules_num - 1));

    if (pop_rule_from_pos(accept_rules, v_tmp, position, acc_rules_num) < 0)
      return -1;
  ...
  case DENIAL:
    ...
    /* allocate space for temporary work */
    v_tmp = (ipfire_rule *)malloc(sizeof(ipfire_rule) * (den_rules_num - 1));

    if (pop_rule_from_pos(denial_rules, v_tmp, position, den_rules_num) < 0)
      return -1;
  ...
  case TRANSLATION:
    ...
    /* allocate space for temporary work */
    v_tmp = (ipfire_rule *)malloc(sizeof(ipfire_rule) * (transl_rules_num - 1));
...
int log_exiting(const struct tm *tm, const char *user,
  ...
  unsigned long long sum_now =
      nls->in_rcv + nls->out_rcv + nls->fwd_rcv + nls->pre_rcv + nls->post_rcv;

  snprintf(initlog, LOGLINELEN, "-%d-%d-%d-%d/%d:%d:%d#TOT:%llu#%s\n",
           tm->tm_wday, tm->tm_mday, tm->tm_mon, tm->tm_year + 1900,
           tm->tm_hour, tm->tm_min, tm->tm_sec, sum_now, user);
  return flog(initlog);
}
