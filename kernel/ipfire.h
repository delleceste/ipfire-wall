...
/* network printk() should be guarded by net_ratelimit() function -- see
 * net/core/utils.c */
#define IPFI_PRINTK(args...)                                                   \
  do {                                                                         \
    if (net_ratelimit())                                                       \
      printk(args);                                                            \
  } while (0)
 ...
 * The variable used by this macro must be globally declared in the file/module
 * that uses it and its name must be `moderate_print. It must be an unsigned int
 * array of dimension MAXMODERATE_ARGS. An example is represented in
 * ipfi_machine, where `moderate_print is declared at the beginning of the file
 * and initialized to 0 values in the initialization method `init_translation()'
 * via the memset function.
 */
#define IPFI_MODERATE_PRINTK(id, args...)                                      \
  do {                                                                         \
    if (id < MAXMODERATE_ARGS) {                                               \
      if (moderate_print[id] % moderate_print_limit[id] == 0) {                \
        moderate_print[id]++;                                                  \
        IPFI_PRINTK(args);                                                     \
        IPFI_PRINTK("[message repeated %u times. Next repetition in %u "       \
...
struct ipfire_options {
  ...
  u8 user_allowed : 1, noflush_on_exit : 1,
      loglevel : 3, /* logging level by means of printk */
