...
int get_original_dest(struct sock *sk, int optval, void __user *user,
                      int *len) {
  ...
  net_quadruplet n4 = {inet->inet_rcv_saddr, inet->inet_daddr, inet->inet_sport,
                       inet->inet_dport, 1};
...
inline int private_address(__u32 addr) {
  ...
  if ((haddr >= 0x0a000000) && (haddr <= 0x0affffff))
    return 1;
