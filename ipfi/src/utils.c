...
ipfire_rule *lookup_rule_by_id(uint32_t id, int *policy) {
  ...
  /* First try the kernel cache - this has rules with proper rulenames */
  cached = lookup_rule_in_cache(id);
  if (cached) {
    /* We can't determine policy from cache alone, so leave it NULL if needed */
    return cached;
  }

  /* Fall back to local userspace arrays */
 ...
 */
int filter_packet_to_print(const ipfire_info_t *p,
  ...
  if (f->direction) {
    if (!((f->in && p->flags.direction == IPFI_INPUT) ||
          (f->out && p->flags.direction == IPFI_OUTPUT) ||
          (f->fwd && p->flags.direction == IPFI_FWD) ||
          (f->pre && p->flags.direction == IPFI_INPUT_PRE) ||
          (f->post && p->flags.direction == IPFI_OUTPUT_POST)))
      return -1;
  }
  /* protocol */
  if (f->protocol) {
    if (!((f->tcp && p->packet.ip.protocol == IPPROTO_TCP) ||
          (f->udp && p->packet.ip.protocol == IPPROTO_UDP) ||
          (f->icmp && p->packet.ip.protocol == IPPROTO_ICMP) ||
          (f->igmp && p->packet.ip.protocol == IPPROTO_IGMP)))
      return -1;
  }
  ...
  if (f->policy) {
    if (!((f->rule->nflags.policy == IPFI_ACCEPT &&
           p->response.verdict == IPFI_ACCEPT) ||
          (f->rule->nflags.policy == IPFI_DROP &&
           p->response.verdict == IPFI_DROP) ||
          (f->rule->nflags.policy == IPFI_IMPLICIT &&
           p->response.verdict == IPFI_IMPLICIT)))
      return -1;
  }
  ...
  if (f->ip) {
    ...
    {
      if (!((f->rule->ip.ipsrc[0] == p->packet.ip.saddr) ||
            (f->rule->ip.ipsrc[0] == p->packet.ip.daddr)))
        return -1;
    }
    if (f->sip && (f->rule->ip.ipsrc[0] != p->packet.ip.saddr))
      return -1;
    if (f->dip && (f->rule->ip.ipdst[0] != p->packet.ip.daddr))
      return -1;
  }
   ...
   */
  if (f->state && p->response.state &&
      !(f->setup | f->setupok | f->est | f->finwait | f->closewait |
  ...
  if (f->port && !f->sport && !f->dport) {
    if (!((htons(f->rule->tp.sport[0]) == sport) ||
  ...
  if (f->device) {
    ...
    if (!f->indevice &&
        !f->outdevice) /* match any between in and out interfaces */
    {
      int match_in = 0;
      int match_out = 0;
...
void copy_servent(struct ipfire_servent *dst, const struct servent *src) {
...
struct ipfire_servent *alloc_and_fill_services_list(void) {
  ...
  se = (struct ipfire_servent *)malloc(sizeof(struct ipfire_servent) *
                                       (nentries + 1));
  if (se == NULL) {
...
int resolv_ports(const struct ipfire_servent *ipfise,
                 const unsigned short protocol, char *srcserv, char *dstserv,
...
int get_service_name(const struct ipfire_servent *ise, char *name, char *proto,
  ...
  strcpy(name, "");
  while (!ise[i].last) {
    if ((!strcmp(ise[i].s_proto, proto)) & (ise[i].s_port == port)) {
...
void get_igmp_type_code(const int t, const int c, char *type, char *code) {
  ...
  case 0x11:
    ...
    case 0:
      ...
      break;
    default:
      snprintf(code, MAXLEN, "/Max response time: %d", c);
      break;
    }
    break;
  case 0x12:
...
int print_packet(const ipfire_info_t *pack,
  ...
  source_addr.s_addr = pack->packet.ip.saddr;
  dest_addr.s_addr = pack->packet.ip.daddr;

  if (pack->flags.state_max_entries) {
    PRED, PUND, printf(TR("WARNING")), PCL,
        printf(TR(": maximum number of entries reached for the state tables."));
    PNL;
    return -1;
  } else if (pack->flags.nat_max_entries) {
    PRED, PUND, printf(TR("WARNING")), PCL,
        printf(TR(": maximum number of entries reached for the NAT tables."));
    PNL;
    return -1;
  } else if (pack->flags.snat_max_entries) {
    PRED, PUND, printf(TR("WARNING")), PCL,
        printf(TR(": maximum number of entries "
                  "reached for the source nat tables"));
    PNL;
    PGRN, printf(TR("HINT"));
    PCL;
    printf(TR(":    in the options, the administrator should"));
    PNL, printf(TR("         increase the maximum number of the source nat "
                   "table entries."));
    PNL;
    return -1;
  }
  ...
  if (ipfi_svent != NULL) {
    resolv_ports(ipfi_svent, pack->packet.ip.protocol, sport_res, dport_res,
                 pack->packet.transport_header.tcphead.source,
                 pack->packet.transport_header.tcphead.dest);
  }

  switch (pack->packet.ip.protocol) {
  case IPPROTO_TCP:
    printf("|TCP| ");
  ...
  case IPPROTO_UDP:
    ...
    break;
  case IPPROTO_ICMP:
    printf("|ICMP| SRC:%s --> DST:%s |", src_address, dst_address);
  ...
  case IPPROTO_IGMP:
    get_igmp_type_code(pack->packet.transport_header.igmphead.igmp_type,
                       pack->packet.transport_header.igmphead.igmp_code,
                       igmp_type, igmp_code);
    igmp_group = pack->packet.transport_header.igmphead.igmp_group;
    inet_ntop(AF_INET, (void *)&igmp_group, igmp_grp_address, INET_ADDRSTRLEN);
  ...
  default:
    ...
    case IPPROTO_GRE:
      // 							printf( "\""
      // GRAY "CISCO GRE (RFC 1701, 1702)" CLR "\"");
      break;
    case IPPROTO_IPV6:
    ...
    case IPPROTO_AH:
      ...
      break;
    case IPPROTO_PIM:
      printf("\"" GRAY "INDEPENDENT MULTICAST" CLR "\"[" GRAY "PIM" CLR "]");
      break;
    case IPPROTO_COMP:
  ...
  if ((pack->response.state) && (pack->response.verdict == IPFI_ACCEPT)) {
