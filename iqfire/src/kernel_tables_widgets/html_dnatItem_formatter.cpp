...
HtmlDnatItemFormatter::HtmlDnatItemFormatter(const QTreeWidgetItem* it)
{
  ...
  d_html += QString("<h3>Destination nat table summary</h3>");
  ...
  if(it->text(DA) == it->text(NEWDA))
    d_html += "<li>destination address is <em>not changed</em> by the rule</li>";
  else
    d_html += QString("<li><em>new</em> destination address: <a href=\"action://resolvenewdip%1\" title=\"Resolve destination address\">%2</a></li>").
    arg(it->text(NEWDA)).arg(it->text(NEWDA));
  ...
  d_html += QString("<li>destination port: <a href=\"action://resolvedport%1\" title=\"Click to obtain service name\">%2</a></li>").
   ...
   if(it->text(DP) == it->text(NEWDP))
    d_html += "<li>destination port <em>not changed</em> by the rule</li>";
  else
    d_html += QString("<li><em>new</em> destination port: <a href=\"action://resolvenewdport%1\" title=\"Click to obtain service name\">%2</a></li>").
    arg(it->text(NEWDP)).arg(it->text(NEWDP));
    
  
  d_html += QString("<li>input interface: <em>%1</em></li><li>output interface: <em>%2</em></li><li>state: <em>%3</em></li>").
    arg(it->text(INIF)).arg(it->text(INIF)).arg(it->text(STATE));
