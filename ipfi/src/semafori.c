...
int ottieni_sem_id(key_t sem_key) {
  ...
  /* se key != 0, l'effetto dipende da flag, nsems indica il numero di semafori
     che deve contenere l'insieme creato
  */
  /* creo il semaforo solo se non esiste gia' */
  /* controllo se il semaforo esiste chiamando semget con nsems =0 e flag =0 */
  /* semget cosi' restituisce l'id del semaforo se esiste, altrimenti fallisce
     con errno = ENOENT se il semaforo non esiste
  */
  if ((sem_id = semget(sem_key, 0, 0)) == -1 &&
      errno == ENOENT) /* non esiste */
  {
    // printf("\e[0;37mVerra' creato il semaforo.\e[00m\n");
    sem_id = semget(
        sem_key, 1,
        IPC_CREAT |
            0660); /* creazione dell'insieme di semafori se non esiste gia'*/
    if (sem_id == -1) {
      perror("semget()");
    ...
    /* a questa creazione corrisponde l'inizializzazione della struttura
       semid_ds, associata all'insieme di semafori, che contiene il numero dei
       semafori dell'insieme, due campi temporali, e una struttura di permessi
       per consentire l'alterazione del semaforo. Ciascun semaforo e'
       reallizzato come una struttura di tipo sem che ne contiene i dati
       essenziali come il valore del semaforo ... Il semaforo va inizializzato:
    */
...
int esiste_semaforo(int sem_key) { return semget(sem_key, 0, 0); }

int valore_semaforo(int sem_id) { return semctl(sem_id, 0, GETVAL); }

int rimuovi_semaforo(int semid) { return (semctl(semid, 1, IPC_RMID < 0)); }

int unlock_sem(int id_sem)
/* void perche' la uso in situazioni non critiche: non controllo lo stato
 * d'uscita, la uso solo per sincronizzare la stampa */
{
  if (impostazione_semaforo(id_sem, SEMVERDE) < 0) {
    perror("\e[1;31munlock_sem() Errore di impostazione del semaforo al "
           "verde\e[1;37m");
    return -1;
  }
  return 1;
 ...
 * d'uscita, la uso solo per sincronizzare la stampa */
{
  if (impostazione_semaforo(id_sem, SEMROSSO) < 0) {
    perror("\e[1;31mlock_sem(): Errore di impostazione del semaforo al "
           "rosso\e[1;37m");
    return -1;
  }
  return 1;
