...
void toupper_username(char *upperun) {
  ...
  if (get_user_info(USERNAME, upperun) < 0) {
    ...
    while ((i < strlen(upperun)) && (i < PWD_FIELDS_LEN - 1)) {
 ...
 * copied in info
 * NOTE: uses geteuid() to get the EFFECTIVE user ID,
 * so running via sudo will use root's paths, not the
 * original user's paths */
int get_user_info(int type, char *info) {
  struct passwd *pwd;
  pwd = getpwuid(geteuid()); /* THE FIX: was getuid() - use geteuid() so sudo
                                uses root's config */
  if (pwd == NULL)
    return -1;
  if (type == USERNAME)
...
int module_already_loaded(void) {
  ...
  if (fp == NULL) {
    PRED, printf(TR("Error opening file %s fo reading"), PROC_MODULES), PNL,
        PNL;
    return -1;
  }
...
void get_rule_name(const char *line, char *name) {
  ...
  while ((line[i] != '\0') && (i < MAXLINELEN) & (j < RULENAMELEN - 1)) {
    name[j] = line[i];
    i++, j++;
  }
  name[j] = '\0';
...
int cidr_to_interval(char *addr) {
  ...
  dotted_decimal = get_address_and_mask(addr, &inetaddress, &inetmask);

  if (dotted_decimal < 0)
    return -1;

  if (dotted_decimal > 0) {
    addr_and_dotted_mask_to_inet_interval(inetaddress, inetmask, &start_address,
                                          &end_address);
  } else {
    addr_and_integer_to_inet_interval(inetaddress, inetmask, &start_address,
                                      &end_address);
  }

  if (addresses_to_string_interval(addr, start_address, end_address) < 0)
    return -1;
...
int fill_ip_interval(const char *addr, ipfire_rule *r, int direction) {
  ...
  while ((i < strlen(addr)) && (isblank(addr[i])))
    i++;
  if (i == strlen(addr)) {
    printf("\e[1;31m* \e[0mcommon.c: fill_ip_interval(): cursor for input "
           "argument would go beyond boundaries!\n");
    return -1;
  }
  /* get first address */
  while ((i < strlen(addr)) && (j < INET_ADDRSTRLEN - 1) &&
         (!isblank(addr[i])) && (addr[i] != '-')) {
    firstaddr[j] = addr[i];
    j++, i++;
  }
  firstaddr[j] = '\0';
  ...
  while ((i < strlen(addr)) && (isblank(addr[i])))
    i++;
  if (i == strlen(addr)) {
    printf("\e[1;31m* \e[0mcommon.c: fill_ip_interval() [step 2]: cursor for "
           "input argument would go beyond boundaries!\n");
    return -1;
  }
  /* finally, get second address */
  j = 0;
  while ((i < strlen(addr)) && (j < INET_ADDRSTRLEN - 1) &&
         (!isblank(addr[i])) && (addr[i] != '\0')) {
    endaddr[j] = addr[i];
    i++, j++;
  }
  endaddr[j] = '\0';
  if (inet_pton(AF_INET, firstaddr, &add1) <= 0) {
    PRED, perror(TR("fill_ip_interval(): error getting first address!"));
    PCL;
    return -1;
  }
  if (inet_pton(AF_INET, endaddr, &add2) <= 0) {
    PRED, perror(TR("fill_ip_interval(): error getting second address!"));
    PCL;
    return -1;
  }
...
/* simple ip */
int fill_plain_address(const char *naddr, ipfire_rule *r, int direction) {
  int i = 0, k = 0, j = 0;
  struct in_addr add;
  ...
  while (k < strlen(naddr) && (!isdigit(naddr[k])))
    k++;
  if (k >= strlen(naddr)) {
    printf("\e[1;31m*\e[0m common.c: fill_plain_address(): passed address "
           "would go beyond boudaries!\n");
    return -1;
  }

  for (i = k, j = 0; i < MAXLINELEN && j < INET_ADDRSTRLEN - 1; i++, j++)
    inetaddr[j] = naddr[i];
  inetaddr[j] = '\0';
...
int addresses_to_string_interval(char *addr, const __u32 starta,
                                 const __u32 enda) {
...
int check_ip_interval(u32 ip1, u32 ip2) {
  ...
  if (ip1 >= ip2) {
    PRED, printf(TR("First address [%s] must be less than second [%s]!"),
                 ...
                 inet_ntop(AF_INET, &ip2, addr, INET_ADDRSTRLEN));
    PNL;
    return -1;
  }
  return 0;
...
int get_address_and_mask(const char *addr, __u32 *address, __u32 *mask) {
  int i = 0, j = 0;
  int dots = 0;
...
int fill_not_ip(const char *naddr, ipfire_rule *r, int direction) {
  ...
  if (direction == SOURCE) {
    ...
    r->ip.ipdst[0] =
        (__u32)add.s_addr; /* DIFFERENT_FROM: fill in first element */
  }
  return 0;
}
...
void remove_exclmark(char *addr) {
  ...
  while (i < strlen(addr) && (!isdigit(addr[i])))
    i++;
  if (i == strlen(addr)) {
    printf("\e[1;31m* \e[0mcommon.c: remove_exclmark(): cursor for input "
           "argument would go beyond boundaries!\n");
    return;
  }
...
int fill_port_interval(const char *port, ipfire_rule *r, int direction) {
  ...
  while ((i < strlen(port)) && (isblank(port[i])))
    i++;
  /* get first port */
  while ((j < 5) && (i < strlen(port)) && (!isblank(port[i])) &&
         (port[i] != '-')) {
    firstport[j] = port[i];
    j++, i++;
  }
  ...
  j = 0;
  while ((i < strlen(port)) && (j < 5) && (!isblank(port[i])) &&
         (port[i] != '\0')) {
    endport[j] = port[i];
    i++, j++;
  }
...
int fill_plain_port(const char *port, ipfire_rule *r, int direction) {
  ...
  while (k < strlen(port) && (!isdigit(port[k])))
    k++;
  if (k >= strlen(port)) {
    printf("\e[1;31m *\e[0mcommon.c: fill_plain_port(): cursor in port "
           "argument would go out of bounds.");
    return -1;
  }

  for (i = k, j = 0; i < MAXLINELEN && j < 5; i++, j++)
    cport[j] = port[i];
  cport[j] = '\0';
...
int check_port_interval(int p1, int p2) {
  if ((p1 < MINPORT) | (p1 > MAXPORT)) {
    PRED,
        printf(TR("Bad value for the first port! Must be between %d and %d!"),
               MINPORT, MAXPORT),
        PNL;
    return -1;
  }
  ...
  if ((p2 < MINPORT) | (p2 > MAXPORT)) {
    PRED,
        printf(TR("Bad value for the second port! Must be between %d and %d!"),
               MINPORT, MAXPORT),
        PNL;
    return -1;
  }
  ...
  if (p1 >= p2) {
    PRED,
        printf(TR("%d, the first port, is not less than %d, the second!"), p1,
               p2),
        PNL;
    return -1;
  }
...
int fill_not_port(const char *port, ipfire_rule *r, int direction) {
  ...
  if (((pt = atoi(port)) < MINPORT) || (pt > MAXPORT)) {
    PRED,
        printf(TR("Value of port must be between %d and %d, not %d!"), MINPORT,
               MAXPORT, pt),
        PNL;
    return -1;
  }
...
int get_string(char *string, const char *line) {
  ...
  if (i == strlen(line) || i == MAXLINELEN) {
    printf("\e[1;31m*\e[0m common.c: get_string(): index of input argument "
           "would go beyond boundaries!\n");
    return -1;
  }
...
int get_string_n(char *string, const char *line, int limit) {
  ...
  if (i == strlen(line) || i == MAXLINELEN) {
    printf("\e[1;31m*\e[0m common.c: get_string_n(): index of input argument "
           "would go beyond boundaries!\n");
    return -1;
  }
  i++; /* pass '=' */
  ...
  /* read the rest of the string containing the address */
  while ((i < strlen(line)) && (j < limit) && (i < MAXLINELEN) &&
         (line[i] != '\n'))
  /* 16 is the value of IFNAMSIZ and 15 is the maximum number
   * of digits of an ipv4 address */
  {
...
int get_integer(const char *line) {
  ...
  while ((i < strlen(line)) && (i < MAXLINELEN) && (line[i] != '='))
    i++;
  if (i == strlen(line) || i == MAXLINELEN) {
    printf("\e[1;31m*\e[0m common.c: get_integer(): index of input argument "
           "would go beyond boundaries!\n");
    return -1;
  }
  i++;
...
int get_options(command *opt, struct userspace_opts *uo, struct cmdopts *cmdo) {
  FILE *fpopt;
  ...
  if ((loglevel < 0) | (loglevel > 7)) {
    printf(RED "KLOGLEVEL must be set to 0 ... 7, not %d! Setting to 7." NL,
           loglevel);
    opt->loglevel = 7;
    cmdo->kloglevel = 7;
  } else {
  ...
  if ((loguser < 0) | (loguser > 7)) {
    printf(RED "LOGUSER must be set to 0 ... 7, not %d! Setting to 7." NL,
           loguser);
    opt->loguser = 7;
    cmdo->loguser = 7;
  } else {
  ...
  if ((uo->loglevel < 0) | (uo->loglevel > 7)) {
    printf(RED "LOGLEVEL must be set to 0 ... 7, not %d! Setting to 7." NL,
           uo->loglevel);
    uo->loglevel = 7;
  }
  return 0;
...
int parse_cmdline(struct cmdopts *cmdo, struct userspace_opts *uo, command *cmd,
  ...
  while (i < argc) {
    if ((!strcmp(argv[i], "-noflush")) || (!strcmp(argv[i], "/noflush"))) {
      if ((user = getuid()) == 0) {
        cmdo->noflush_on_exit = 1;
        ...
        PVIO,
            printf(TR("Option \"%s\": warning: you are user %d."), argv[i],
                   user),
            PNL, printf(TR("You must be root to let rules active in firewall")),
            PNL, printf(TR("after turning off userspace interface!")), PNL;
    }
    /* read /etc/services and resolv ports into names */
    else if ((!strcmp(argv[i], "-services")) || (!strcmp(argv[i], "/services")))
      uo->resolv_services = 1;
    else if ((!strcmp(argv[i], "-mailer")) || (!strcmp(argv[i], "/mailer"))) {
      if (i + 2 >= argc) {
        PRED, printf(TR("Parameter \"%s\" must be followed by an integer"),
                     argv[i]);
      ...
      if (i + 1 >= argc) {
        PRED, printf(TR("Parameter \"%s\" must be followed by an integer from "
                        "0 to 7"),
                     argv[i]);
        PNL;
        return -1;
      ...
      if (i + 1 >= argc) {
        printf(
            TR("Parameter \"%s\" must be followed by an integer from 0 to 7"),
            argv[i]);
        PNL;
        return -1;
      }
    ...
    else if ((!strcmp(argv[i], "-dns")) || (!strcmp(argv[i], "/dns"))) {
      if (i + 1 >= argc) {
        PRED;
        printf(
            TR("Parameter \"%s\" must be followed by an integer indicating,"),
            argv[i]);
        PNL;
        printf(TR("in seconds, the refresh time."));
        PNL;
        return -1;
      }
      uo->dns_resolver = 1;
      ...
      i = i + 1;
    } else if ((!strcmp(argv[i], "-nodns")) || (!strcmp(argv[i], "/nodns")))
      uo->dns_resolver = 0;
      ...
      if (i + 1 >= argc) {
        PRED, printf(TR("Parameter \"%s\" must be followed by a file name:"),
                     argv[i]);
        PNL;
        printf(TR("e.g.: \"/var/log/ipfire.log\""));
        PNL;
        return -1;
      }
      if (getuid() == 0) {
      ...
      *different_ruleset_by_cmd = 1;
      i = i + 1;
    } else if ((!strcmp(argv[i], "-lang")) || (!strcmp(argv[i], "/lang"))) {
      if (i + 1 >= argc) {
        PRED, printf(
                  TR("Parameter \"%s\" must be followed by a string containing "
                     "a filename with translated strings, positioned in the "
                     ".IPFIRE/languages directory of the home user."),
                  argv[i]);
        PNL;
        return -1;
      }
    ...
    else if ((!strcmp(argv[i], "-nouser")) || (!strcmp(argv[i], "/nouser"))) {
      if ((user = geteuid()) == 0) {
        cmdo->user_allowed = 0;
        cmd->user_allowed = 0;
      } else
     ...
     * Implies "-load" */
    else if ((!strcmp(argv[i], "-rc")) || (!strcmp(argv[i], "/rc")) ||
             (!strcmp(argv[i], "rc"))) {
      if ((user = geteuid()) == 0) {
        uo->justload = 1;
        uo->rc = 1;
        ...
        PVIO,
            printf(TR("Option \"%s\": warning: you are user %d."), argv[i],
                   user),
            printf(
                TR("You must be root to set \"justload\" or \"rc\" options.")),
            PNL;

    } else if ((!strcmp(argv[i], "-flush")) || (!strcmp(argv[i], "/flush")) ||
               (!strcmp(argv[i], "flush"))) {
      if ((user = geteuid()) == 0) {
        uo->flush = 1;
        cmdo->noflush_on_exit = 0;
    ...
    } else if ((!strcmp(argv[i], "-rmmod")) || (!strcmp(argv[i], "/rmmod")) ||
               (!strcmp(argv[i], "rmmod"))) {
      if ((user = geteuid()) == 0) {
        uo->rmmod = 1;
      } else
...
int send_to_kernel(void *mess, const struct netl_handle *handle,
  ...
  p = fill_payload(messhead, mess,
                   len); /* memcpy does not return error codes */
...
int build_hello_command(command *cmdh, char *argv0) {
  cmdh->cmd = HELLO;
  memset(&cmdh->content.fwsizes, 0, sizeof(struct firesizes));
...
int write_rule(FILE *fp, const ipfire_rule arule, int index) {
  ...
  if ((arule.direction == NODIRECTION) && ((arule.nat) | (arule.masquerade)))
    PNL, PNL, PRED,
        printf(TR("ERROR: MASQUERADE/NAT RULE AND NO DIRECTION SPECIFIED! "
                  "[RULE%d]"),
               index + 1),
        PNL, PNL;
  ...
  if ((arule.nflags.src_addr == ONEADDR) && (arule.parmean.samean == MULTI)) {
    ...
    fprintf(fp, "SRCADDR=%s\n",
            inet_ntop(AF_INET, (struct in_addr *)&arule.ip.ipsrc[0], address,
                      INET_ADDRSTRLEN));
    /* other addresses */
    for (i = 1; i < MAXMULTILEN && arule.ip.ipsrc[i] != 0; i++)

      fprintf(fp, "SRCADDR%d=%s\n", i + 1,
              inet_ntop(AF_INET, (struct in_addr *)&arule.ip.ipsrc[i], address,
                        INET_ADDRSTRLEN));
  }
  /* multiple source addresses, but different from */
  else if ((arule.nflags.src_addr == ONEADDR) &&
    ...
    fprintf(fp, "SRCADDR=%s\n",
           ...
           (arule.parmean.samean == DIFFERENT_FROM))
    fprintf(fp, "SRCADDR_NOT=%s\n",
            inet_ntop(AF_INET, &arule.ip.ipsrc[0], address, INET_ADDRSTRLEN));
  else if ((arule.nflags.src_addr == MYADDR) &&
  ...
  if ((arule.nflags.src_addr == ONEADDR) && (arule.parmean.samean == INTERVAL))
    fprintf(fp, "_END_SRCADDR=%s\n",
            inet_ntop(AF_INET, (struct in_addr *)&arule.ip.ipsrc[1], address,
                      INET_ADDRSTRLEN));
  ...
  if ((arule.nflags.dst_addr == ONEADDR) &&
      (arule.parmean.damean == MULTI)) /* start with multi */
  {
    /* first address, in [0] */
    fprintf(fp, "DSTADDR=%s\n",
            inet_ntop(AF_INET, (struct in_addr *)&arule.ip.ipdst[0], address,
                      INET_ADDRSTRLEN));
    /* other addresses */
    for (i = 1; i < MAXMULTILEN && arule.ip.ipdst[i] != 0; i++)
      fprintf(fp, "DSTADDR%d=%s\n", i + 1,
              inet_ntop(AF_INET, (struct in_addr *)&arule.ip.ipdst[i], address,
                        INET_ADDRSTRLEN));
  }
  /* multiple destination addresses, but different from */
  else if ((arule.nflags.dst_addr == ONEADDR) &&
  ...
  else if ((arule.nflags.dst_addr == MYADDR) &&
  ...
  if (arule.nat) {
    if (arule.nflags.newaddr)
      fprintf(fp, "NEWADDR=%s\n",
              inet_ntop(AF_INET, &arule.newaddr, address, INET_ADDRSTRLEN));
...
void write_header(FILE *fp, int whichfile) {
  if (whichfile == ACCEPT) {
    ...
    fprintf(fp, "# DENIAL RULES ARE READ BY FIREWALL FIRST.\n"
    ...
    fprintf(fp, "# HERE SOURCE AND DESTINATION NAT RULES\n"
  ...
  fprintf(fp, "# THIS FILE IS WRITTEN BY IPFIRE-wall PROGRAM.\n"
