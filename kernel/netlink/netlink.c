...
int send_data_to_user(struct sk_buff *skb, pid_t destination_pid,
                      struct sock *socket) {
  int ret = -1;
...
pid_t get_sender_pid(const struct sk_buff *skbff) {
  ...
  if (nlh == NULL) {
    IPFI_PRINTK("IPFIRE: get_sender_pid(): error extracting nlmsghdr from "
                "socket buffer. Cannot determine header pid\n");
    ret = 0;
  } else {
    ...
    if (1) {
      ...
      if (credentials_pid != header_pid) {
        IPFI_PRINTK("IPFIRE: PID mismatch! Did you try to fool me? :)\nIPFIRE: "
                    "CREDENTIALS PID: %u, HEADER PID: %u\n",
