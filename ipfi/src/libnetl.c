...
/* function prototypes */
static int netl_send_to_kernel(const struct netl_handle *h, const void *msg,
                               size_t len);

static int cceive_from_kernel(const struct netl_handle *h, void *buf,
                              size_t len);
...
static int netl_send_to_kernel(const struct netl_handle *h, const void *msg,
                               size_t len) {
  int status =
      sendto(h->fd, msg, len, 0, (struct sockaddr *)&h->peer, sizeof(h->peer));
  if (status < 0) {
...
/* uses netlink macros to check and access received buffer and extract data from
 * it. Extracts len bytes from the receive buffer netlink payload and copies
 * them into buf.
 */
static int netl_receive_from_kernel(const struct netl_handle *h, void *buf,
                                    size_t len) {
  int status;
  ...
  if (addrlen != sizeof(h->peer)) /* must be equal */
  {
    ...
    syslog(LOG_ERR,
           "libnetl.c: netl_receive_from_kernel(): addrlen ! sizeof(h->peer)");
    return -1;
  }
  if (h->peer.nl_pid != 0) /* kernel pid = 0 */
  {
    ...
    syslog(LOG_ERR,
           "libnetl.c: netl_receive_from_kernel(): pid mismatch!\n"
           "pid read: %d, my pid: %d\n",
           h->peer.nl_pid, getpid());
  ...
  if (status == 0) /* recvfrom got 0 bytes */
  {
    ...
    syslog(LOG_ERR,
           "libnetl.c: netl_receive_from_kernel(): recvfrom() got 0 bytes!");
    return -1;
  }
...
struct netl_handle *
alloc_netl_handle(int protocol) /* returns a mallocated handle or NULL */
{
  ...
  /* Increase receive buffer size (needs to be done before bind or connect
  ...
  /* bind() */
  if (bind(netlh->fd, (struct sockaddr *)&netlh->local, sizeof(netlh->local)) <
      0) {
    perror("bind() error");
  ...
  netlh->peer.nl_family = AF_NETLINK; /* This was missing or zeroed out! */
  netlh->peer.nl_pid = 0;             /* Kernel pid is 0 */
  netlh->peer.nl_groups = 0;          /* Unicast */
...
int read_from_kern(const struct netl_handle *h, unsigned char *buf,
                   size_t len) {
 ...
 */
void *fill_payload(struct nlmsghdr *nlhmess_header, const void *data,
                   size_t len) {
