...
next_drop_rule:;
    ...
    list_for_each_entry_rcu(rule, &allowed->list, list) {
        ...
        int in_lo = 0, out_lo = 0;
        if(flow->in != NULL && strcmp(flow->in->name, "lo") == 0)
            in_lo = 1;
        if(flow->out != NULL && strcmp(flow->out->name, "lo") == 0)
            out_lo = 1;

        if ((res = device_filter(rule, flow->in, flow->out)) < 0) {
            if(in_lo) {
                printk("loopback in: failed device filters\n");
            }
            if(out_lo) {
                printk("loopback out: failed device filters\n");
            }
            goto next_pass_rule;
        }
        else if (res > 0)
...
struct state_table *keep_state(const struct sk_buff *skb,
    ...
    struct state_table *state_t =
            ...
            /* Debug: Log ICMP state creation */
            if (state_t->protocol == IPPROTO_ICMP) {
                struct iphdr *iph = ip_hdr(skb);
                printk("ICMP STATE CREATE: %pI4->%pI4 dir=%d in_if=%d out_if=%d\n",
                       &iph->saddr, &iph->daddr, flow->direction, state_t->in_ifindex,
                       state_t->out_ifindex);
            }
...
int add_ftp_dynamic_rule(struct state_table *ftpt) {
    ...
    if (ftpt->ftp != FTP_DEFINED) {
        IPFI_PRINTK("IPFIRE: ftp support: you shouldn't be here without "
                    "FTP_DEFINED set!\n");
        return -1;
    }
