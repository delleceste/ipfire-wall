...
struct sk_buff *build_packet(void *buf, int numbytes) {
  ...
  if (NLMSG_OK(nlh, NLMSG_LENGTH(numbytes))) {
    ...
    printk("IPFIRE: NLMSG_OK failed: NLMSG_LENGTH(%d) is %d, "
  ...
  printk("IPFIRE: nlmsg failure in build_packet() - NLMSG_SPACE:%dbytes - "
         "netlink header:%lubytes - data length:%dbytes - NLMSG_LENGTH(data "
         "len): %dbytes - ipfi_netl_packet_builder.c\n",
...
struct sk_buff *build_info_t_nlmsg(const struct sk_buff *skb,
                                   ...
                                   const struct info_flags *flags, int *err) {
  ...
  if (!nl_skb) {
    *err = ENOMEM;
    ...
    case IPPROTO_TCP:
      memcpy(&msg->packet.transport_header.tcphead, (void *)iph + iph->ihl * 4,
             sizeof(struct tcphdr));
      break;
    case IPPROTO_UDP:
      memcpy(&msg->packet.transport_header.udphead, (void *)iph + iph->ihl * 4,
             sizeof(struct udphdr));
      break;
    case IPPROTO_ICMP:
      memcpy(&msg->packet.transport_header.icmphead, (void *)iph + iph->ihl * 4,
             sizeof(struct icmphdr));
      break;
    case IPPROTO_IGMP:
      memcpy(&msg->packet.transport_header.igmphead, (void *)iph + iph->ihl * 4,
