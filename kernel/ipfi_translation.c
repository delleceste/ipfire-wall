...
extern int state_machine(const ipfire_info_t *info, 
		int current_state, short reverse);
		...
		struct sockaddr_in *sin)
{
	...
	if ((dnt->old_saddr == iit->iphead.daddr) &&
			(dnt->new_daddr == iit->iphead.saddr) &&
			(dnt->old_sport == iit->transport_header.tcphead.dest) &&
			(dnt->new_dport == iit->transport_header.tcphead.source))
	{
...
int get_original_dest(struct sock *sk, int optval, void __user * user, int *len)
{
	...
	*/
	...
	{
		IPFI_PRINTK("SO_ORIGINAL_DST: len %u not %u\n", *len, sizeof(struct sockaddr_in));
		return -EINVAL;
	}

	/* b. allocate necessary data structures */
	tmp_iit = (ipfire_info_t *) kmalloc(sizeof(ipfire_info_t), GFP_ATOMIC);
	sin = (struct sockaddr_in *) kmalloc(sizeof(struct sockaddr_in), GFP_ATOMIC);

	if(tmp_iit == NULL || sin == NULL) /* check memory allocation */
	{
		IPFI_PRINTK("IPFIRE: memory allocation error in get_original_dest()\n");
		return -ENOMEM;
	}
...
net_quadruplet get_quad_from_skb(const struct sk_buff* skb)
{
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,22) /* support old kernels */
	iphdr = skb->nh.iph;
#else
	iphdr = ip_hdr(skb);
#endif
	...
	switch (iphdr->protocol)
	{
		case IPPROTO_TCP:
			...
			netquad.sport = ptcphead->source;
			netquad.dport = ptcphead->dest;
			break;
		case IPPROTO_UDP:
			pudphead = skb_header_pointer(skb, iphdr->ihl * 4, sizeof(udphead), &udphead);
			
			if (check_udp_header_from_skb(skb, pudphead) < 0)
			    return nq_invalid;
			
			netquad.sport = pudphead->source;
			netquad.dport = pudphead->dest;
			break;
		case IPPROTO_ICMP:
		case IPPROTO_IGMP:
		case IPPROTO_GRE:
		...
		__u32 daddr, __u16 dport, struct pkt_manip_info mi)
{
	...
	if(mi.sp ^ mi.dp)
	{
	  switch (ipheader->protocol)
	  {
		  case IPPROTO_TCP:
			  if (check_tcp_header_from_skb(skb, ptcphead) < 0)
				  return network_header_null("manip_skb()", "malformed TCP header");
		  ...
		  case IPPROTO_UDP:
			  if (check_udp_header_from_skb(skb, pudphead) < 0)
				  return network_header_null("manip_skb()", "malformed UDP header");
		...
		const ipfire_rule * dnat_rule)
{
	...
	dnentry->new_daddr = original_pack->iphead.daddr;
	/* copy interfaces names */
	strncpy(dnentry->in_devname, original_pack->devpar.in_devname,
			IFNAMSIZ);
	strncpy(dnentry->out_devname, original_pack->devpar.out_devname,
			IFNAMSIZ);
	switch (original_pack->iphead.protocol)
	{
		case IPPROTO_TCP:
			dnentry->old_sport =
				original_pack->transport_header.tcphead.source;
			dnentry->old_dport =
				original_pack->transport_header.tcphead.dest;
			dnentry->new_dport = dnentry->old_dport;
			break;
		case IPPROTO_UDP:
			dnentry->old_sport =
				original_pack->transport_header.udphead.source;
			dnentry->old_dport =
				original_pack->transport_header.udphead.dest;
			dnentry->new_dport = dnentry->old_dport;
			break;
		case IPPROTO_ICMP:
		case IPPROTO_IGMP:
		...
		*dnentry)
{
	if (strlen(dnentry->rulename) > 0)
		strncpy(packet->rulename, dnentry->rulename, RULENAMELEN);
}
#endif
		...
		const struct dnatted_table *dne2)
{
	return ((dne1->protocol == dne2->protocol) &&
			...
			(!(strcmp(dne1->in_devname, dne2->in_devname))) &&
			(!(strcmp(dne1->out_devname, dne2->out_devname))));
...
#ifdef ENABLE_RULENAME
	fill_rulename_dnat_entry(original_pack, dnatted_entry);
#endif
...
inline void copy_rulename_from_rule_to_ipfire_info
	(ipfire_info_t * iit_dest, const ipfire_rule * rule_src) {
		if (strlen(rule_src->rulename) > 0)
			strncpy(iit_dest->rulename, rule_src->rulename,
					RULENAMELEN);
	}
#endif
...
int translation_rule_match(const ipfire_info_t * packet, const ipfire_rule * r)
{
    ...
    if (r->nflags.indev)
    {
        if (strcmp
                (r->devpar.in_devname,
                 packet->devpar.in_devname) != 0)
			return -1;
	}
	if (r->nflags.outdev)
	{
		if (strcmp
				(r->devpar.out_devname,
				 packet->devpar.out_devname) != 0)
			return -1;
	}
	/* ip */
    if (address_match(&packet->iphead, r, packet->direction, NULL, NULL) < 0)
		return -1;
	/* transport */
	switch (packet->protocol)
	{
		case IPPROTO_TCP:
            if (port_match(&packet->transport_header.tcphead, NULL, r, IPPROTO_TCP) < 0)
				return -1;
			break;
		case IPPROTO_UDP:
            if (port_match(&packet->transport_header.udphead, NULL, r, IPPROTO_UDP) < 0)
				return -1;
			break;
		case IPPROTO_ICMP:
		case IPPROTO_IGMP:
...
// #ifdef ENABLE_RULENAME
// 	copy_rulename_from_rule_to_ipfire_info(packet, r);
// #endif
	return 1;
}
		...
		const struct snatted_table *sne2)
{
	return ((sne1->protocol == sne2->protocol) &&
			...
			(!(strcmp(sne1->in_devname, sne2->in_devname))) &&
			(!(strcmp(sne1->out_devname, sne2->out_devname))));
		...
		const ipfire_rule * snat_rule)
{
	...
	/* initialize new address to the value in original packet */
	snentry->new_saddr = original_pack->iphead.daddr;
	strncpy(snentry->in_devname, original_pack->devpar.in_devname,
			IFNAMSIZ);
	strncpy(snentry->out_devname, original_pack->devpar.out_devname,
			IFNAMSIZ);
	switch (original_pack->iphead.protocol)
	{
		case IPPROTO_TCP:
		...
		ipfire_info_t * packet)
{
	...
	fill_rulename_snat_entry(packet, snatted_entry);
#endif
	spin_lock_bh(&snat_list_lock);
...
success:
#ifdef ENABLE_RULENAME
	fill_packet_with_snentry_rulename(packet, snt);
#endif
	return 1;
}
...
int check_checksums(const struct sk_buff *skb)
{
	...
	switch (iph->protocol)
	{
		case IPPROTO_TCP:
			...
			/* check header not null or malformed */
			if (check_tcp_header(th, datalen) < 0)
				return -BAD_TCPHEAD_CHECK;
			...
			break;
		case IPPROTO_UDP:
			uh = skb_header_pointer(skb, iph->ihl * 4, sizeof(udphead), &udphead);

			/* check header not null or malformed */
			if (check_udp_header(uh, datalen) < 0)
				return -BAD_UDPHEAD_CHECK;
			/* UDP checksum */
