...
int compare_nat_entries(const struct nat_table *a, const struct nat_table *b)
{
	return (a->type == b->type) &&
	       ...
	       (a->old_saddr == b->old_saddr) &&
	       (a->old_daddr == b->old_daddr) &&
	       (a->old_sport == b->old_sport) &&
	       (a->old_dport == b->old_dport) &&
	       (a->new_addr == b->new_addr) &&
	       (a->new_port == b->new_port) &&
			  ...
			  const ipfire_rule *rule, enum nat_type type)
{
	...
	if (type == NAT_SNAT) {
		...
		entry->new_port = (iph->protocol == IPPROTO_TCP ||
...
void handle_nat_entry_timeout(struct timer_list *t)
{
	...
	ipfi_entry_remove(h, &nat_counters[type]);
	...
	list_for_each_entry_rcu(tmp, &nat_lists[type], h.lnode) {
		if (forward_nat_match(tmp, skb) > 0) {
			if (nat_hold_rcu(tmp)) { // hold the entry (released by the caller)
				...
				ipfi_entry_update_timer(&tmp->h, tmp->protocol,
							tmp->state);
				rcu_read_unlock_bh();
				return tmp;
			}
