...
int print_request(const struct netl_handle *nh_control) {
  ...
  if (str_filter == NULL)
    ...
    if (filter == NULL)
      PVIO, PUND, printf(TR("No filter will be applied."));
    PNL;
    free(str_filter); /* we do not need the string any more: we now have
                         "filter" */
  }

  if (send_to_kernel((void *)&print_req_cmd, nh_control, CONTROL_DATA) < 0)
    PRED, printf(TR("print_request(): ERROR SENDING PRINT REQUEST TO KERNEL!")),
        PNL;
...
int print_command(const command *cmd) {
  ...
  if (cmd->cmd == ADDING_FAILED)
    PRED, printf(TR("Rule adding/removing failed: generic error from kernel!")),
        PNL;
  else if (cmd->cmd == ADDING_FAILED_NORIGHTS)
    PRED,
        printf(TR("Rule adding/removing failed. You have not the permission to "
                  "make such change.")),
        PNL;
  ...
  if (cmd->is_rule) {
    if (!(cmd->cmd == RULE_ALREADY_PRESENT))
      printf(GRAY "- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - "
                  "- - - - - - -" NL);
    PGRN, printf(TR("RULE")), PCL,
        printf(TR(" [owner: %d]:"), cmd->content.rule.owner);
    PNL;
  ...
  if (send_command_to_kernel(&cmdsizes) < 0)
    printf(
        RED
        "Error sending to the kernel the request for the structures sizes!" NL
            NL);
  if (read_command_from_kernel(&cmdsizes) < 0)
    printf(
        RED
        "Error reading from the kernel the request for the structures sizes!" NL
            NL);
  ...
  printf(TR("KERNEL LOGLEVEL: %d LOG KERNEL/USER LEVEL: %d"), cmd->loglevel,
         cmd->loguser),
      PNL, PNL;
  ...
  if (cmd->noflush_on_exit)
    ...
    printf(TR("- Your rules will be removed as soon as you turn off user "
              "interface.")),
        PNL;

  printf(TR("- Destination nat tables lifetime: %lu seconds, max %lu entries."),
         cmd->dnatted_lifetime, cmd->max_nat_entries),
      PNL;
  PGRAY,
      printf(TR("  DNat table size: %u bytes -> max memory for nat tables: "
                "%.2f kB"),
             dnatt_size, (float)(cmd->max_nat_entries * dnatt_size) / 1024),
      PNL;
  printf(TR("- Source nat tables lifetime: %lu seconds, max %lu entries."),
         cmd->snatted_lifetime, cmd->max_nat_entries),
      PNL;
  PGRAY,
      printf(TR("  SNat table size: %u bytes -> max memory for snat tables: "
                "%.2f KB"),
             snatt_size, (float)(cmd->max_nat_entries * snatt_size) / 1024),
      PNL;
  printf(TR("- State conn. tables lifetime: %lus. Max %lu entries. "
            "[Setup/shutdown:%lus.]"),
         cmd->state_lifetime, cmd->max_state_entries,
         cmd->setup_shutd_state_lifetime),
      PNL;
  PGRAY,
      printf(TR("  State table size: %u bytes -> max memory for state tables: "
                "%.2f KB"),
             statet_size, (float)(cmd->max_state_entries * statet_size) / 1024),
      PNL;
  if (cmd->loguser == 1) {
    printf(TR("- Loginfo entries lifetime: %lu, max %lu entries."),
           cmd->loginfo_lifetime, cmd->max_loginfo_entries),
        PNL;
    PGRAY,
        printf(TR("  Loginfo table size: %u bytes -> max memory for loginfo "
                  "tables: %.2f KB"),
               logt_size, (float)(cmd->max_loginfo_entries * logt_size) / 1024),
        PNL;
  } else {
...
int state_table_request(const struct netl_handle *nh_control) {
  ...
  int bytes_read;
  printf(NL "\t- - - - " UNDERL);
  printf(TR("TABLE OF STATEFUL CONNECTIONS"));
  printf(CLR " - - - -" NL NL);
  ...
  printf(NL NL "\t   - - - - ");
  printf(TR("End of connection table"));
  printf(" - - - -" NL);
  return 0;
}
...
int dnat_table_request(const struct netl_handle *nh_control) {
  ...
  int bytes_read;
  printf(NL "\t- - - - " UNDERL);
  printf(TR("TABLE OF ACTIVE DNAT CONNECTIONS"));
  printf(CLR " - - - -" NL NL);
  init_command(&table_req);
  ...
  if (counter == 1)
    PGRAY, printf(TR("Kernel table empty.")), PCL;
  printf(NL NL "\t   - - - - ");
  printf(TR("End of dnat connection table"));
  printf(" - - - -" NL);
  return 0;
}
...
int snat_table_request(const struct netl_handle *nh_control) {
  ...
  int bytes_read;
  printf(NL "\t- - - - " UNDERL);
  printf(TR("TABLE OF ACTIVE SNAT CONNECTIONS"));
  printf(CLR " - - - -" NL NL);
  init_command(&table_req);
  ...
  if (counter == 1)
    PGRAY, printf(TR("Kernel table empty.")), PCL;
  printf(NL NL "\t   - - - - ");
  printf(TR("End of snat connection table"));
  printf(" - - - -" NL);
  return 0;
}
...
int change_smart_log(int level) {
  ...
  if (level != SMART_SIMPLE && level != SMART_STATE) {
    PRED,
        printf(TR("Changing levels different from SMART_SIMPLE or SMART_STATE "
                  "not allowed")),
        PNL, PNL;
    return -1;
...
int print_state_table_entry(const struct state_info *tr, int counter) {
  ...
  inet_ntop(AF_INET, (void *)&tr->saddr, saddr, INET_ADDRSTRLEN);
  inet_ntop(AF_INET, (void *)&tr->daddr, daddr, INET_ADDRSTRLEN);
  printf("* %d.", counter);

  /* Look up rule name from cache */
  ipfire_rule *rule = lookup_rule_in_cache(tr->rule_id);
  if (rule && rule->rulename[0] != '\0') {
    printf(TR("from rule '" CYAN "%s" CLR "' "), rule->rulename);
  } else {
...
int print_dnat_table_entry(const struct dnat_info *di, int counter) {
  ...
  char saddr[INET_ADDRSTRLEN];
  char daddr[INET_ADDRSTRLEN];
  inet_ntop(AF_INET, (void *)&di->saddr, saddr, INET_ADDRSTRLEN);
  inet_ntop(AF_INET, (void *)&di->daddr, daddr, INET_ADDRSTRLEN);
  inet_ntop(AF_INET, (void *)&di->newdaddr, newdaddr, INET_ADDRSTRLEN);
...
int print_snat_table_entry(const struct snat_info *di, int counter) {
  ...
  inet_ntop(AF_INET, (void *)&di->saddr, saddr, INET_ADDRSTRLEN);
  inet_ntop(AF_INET, (void *)&di->daddr, daddr, INET_ADDRSTRLEN);
  inet_ntop(AF_INET, (void *)&di->newsaddr, newsaddr, INET_ADDRSTRLEN);
...
void print_loguser_meanings(int level) {
  if (level >= 6)
    ...
    printf(TR("  [All filtering is monitored, except pre/post directions.]")),
        PNL;
  else if (level >= 4)
...
void print_hello_error(const command *hello) {
  ...
  case IPFIRE_BUSY:
    printf(VIOLET "ipfire is busy!" NL VIOLET
                  "Another instance of an userspace interface seems to be "
                  "running, with pid %d!" NL NL,
           hello->anumber);
    break;
  default:
...
void print_cmd_options(const struct cmdopts *cmdo,
  ...
  if (verbose != 0) {
    printf(TR("- log level of kernel messages: %d;"), cmdo->kloglevel);
    PNL;
    printf(TR("- log level of user interface: %d."), cmdo->loguser);
    PNL;
    print_loguser_meanings(cmdo->loguser);
    printf(TR("- log level on file %s: %u."), uo->logfile_name, uo->loglevel);
    PNL;
  }
  if (cmdo->quiet) {
    printf(TR("- Quiet modality enabled."));
    PNL;
    if (cmdo->loguser > 2)
      printf(TR("  Decrease loguser to avoid message exchange between"));
    PNL;
    printf(TR("  user and kernel space."));
    PNL;
  }
  if (cmdo->noflush_on_exit)
    printf("- "), PGRN, printf(TR("Rules will remain ")), PUND,
        printf(TR("active")), PCL, PGRN,
        printf(TR(" in firewall after you shut")), PNL,
        printf(TR("  down user interface.")), PNL;
  if (cmdo->daemonize)
    printf(NL "- Program will run as a " UNDERL GREEN "daemon" CLR ", although"
              " redirecting\n  output to this console" NL);
  if (cmdo->quiet_daemonize)
    PNL, printf(TR("- Program will run as a ")), PUND, PGRN,
        printf(TR("daemon")), PCL, printf(TR(", redirecting")), PNL,
        printf(TR("output to ")), PUND, PRED, printf(TR("null")), PCL,
        printf(TR(" device.")), PNL;
  if (uo->clearlog)
    PNL, printf(TR("- Log file will be ")), PUND, PVIO, printf(TR("cleared")),
        PCL, printf("." NL);
  if (!uo->rmmod) {
    ...
    printf(TR("- Kernel module will be ")), PVIO, printf(TR("unloaded")), PCL,
        printf(TR(" at exit.")), PNL;
  ...
  if (verbose) /* Print file names only if verbose is not 0 */
  {
    printf(NL);
    printf(UNDERL);
    printf(TR("CONFIGURATION FILES:"));
    printf(NL);
    printf(TR("Permission rules:"));
    PTAB;
    printf("\"%s\".", uo->permission_filename);
    printf(NL);
    printf(TR("Denial rules:"));
    PTAB, PTAB;
    printf("\"%s\".", uo->blacklist_filename);
    printf(NL);
    printf(TR("Translation rules:"));
    PTAB, PTAB;
    printf("\"%s\".", uo->translation_filename);
    printf(NL);
  }
  if (uo->dns_resolver) {
    printf(TR("- Blacklisted sites will be blocked. List will be refreshed "
              "every %d seconds."),
           uo->dns_refresh),
        PNL;
    if (verbose) {
      printf(TR("- Blocked sites rules:")), PTAB,
          printf("\"%s\".", uo->blacksites_filename), printf(NL NL);
    }
  }
...
prompt:
  ...
  printf(TR("Type 'p' if  you want to specify the position of the new rule"));
  PNL;
  printf(TR("in the list, another key to insert new rule at the tail of the"));
  PNL;
  ...
  case 'p':
    PNL;
    printf(TR("Position of the new rule: "));
    if (get_line(line) > 0) {
      new_pos = atoi(line);
      if (new_pos == 0) {
        PNL, PRED,
            printf(TR("Position %d not valid! Minimum position value is 1."),
                   new_pos);
        PNL, PNL;
        goto prompt;
      } else {
        if (add_rule_at_position(r, new_pos) < 0) {
          PRED, printf(TR("Error inserting rule at position %d!"), new_pos);
          PNL;
          return -1;
        } else
          return new_pos;
  ...
  default:
    PNL;
    printf(TR("Rule will be inserted at the end of the list..."));
...
int manage_deleting_rule(ipfire_rule *r) {
  ...
  if (getuid() == 0)
    ...
    PNL, PNL, printf(TR("TYPE 'p' TO DELETE A PERMISSION RULE")), PNL,
        printf(TR("     'd' TO DELETE A DENIAL RULE")), PNL, printf("[p, d]");
  ...
  PNL, printf(
           TR("ENTER THE POSITION OF THE RULE YOU WANT TO DELETE, 0 TO EXIT."));
  PNL;
  switch (scelta) {
  case 'p':
    printf("[");
    ...
    if ((pos <= 0) | (pos > acc_rules_num)) {
      PNL;
      PRED;
      printf(TR("Error: value %d out of range!"), pos);
      return -1;
    }

    PNL, PRED, printf(TR("Rule %d:"), pos);
    PNL;
    PNL;
    print_rules(accept_rules + pos - 1, 1, NULL);
    PNL, PRED, printf(TR("WILL BE DELETED, ARE YOU SURE [y | n]?"));
    PNL;
...
void print_help(void) {
  ...
  if (fphelp == NULL)
    ...
    while (fgets(line, 1024, fphelp) != NULL) {
      ...
      if (count % 6 == 0) {
        PGRAY,
            printf(TR("- - - - - - - Press a key to read on... - - - - - - ")),
            PCL;
        g_getchar();
        printf("\r                                                             "
               "             \r");
...
int filter_rule(const ipfire_rule *rule, const ipfire_rule_filter *filter) {
  ...
  if (filter->sip) /* a source IP is specified */
  {
    ...
    if (filter->rule->nflags.src_addr == MYADDR &&
        rule->nflags.src_addr == MYADDR)
      goto ipdst;
...
ipdst:
  if (filter->dip) /* a destination IP is specified */
  {
    ...
    if (filter->rule->nflags.dst_addr == MYADDR &&
        rule->nflags.dst_addr == MYADDR)
      goto sport;
...
int print_rules(const ipfire_rule *v_rules, int numrules,
  ...
  for (i = 0; i < numrules; i++) {
    ...
    if (filter == NULL || filter_rule(&v_rules[i], filter) >= 0) {
      if (v_rules[i].nflags.policy == DENIAL) {
        PRED, printf("----------------------------------------------");
        PNL;
        PUND, PRED, printf(TR("DENIAL RULE")),
            printf(CLR " N. %d\t", v_rules[i].position);
      } else if (v_rules[i].nflags.policy == ACCEPT) {
        PGRN, printf("----------------------------------------------");
        PNL;
        PUND, PGRN, printf(TR("PERMISSION RULE")),
            printf(CLR " N. %d\t", v_rules[i].position);
      } else if (v_rules[i].nflags.policy == BLACKSITE) {
        PRED, printf("----------------------------------------------");
        PNL;
        PUND, PRED, printf(TR("BLACKLISTED SITE RULE")),
            printf(CLR " N. %d\t", v_rules[i].position);
        ...
        if (v_rules[i].masquerade)
          ...
          printf(TR("OUTPUT DNAT. RULE N. %d"), v_rules[i].position), PTAB;
        else if ((v_rules[i].nat) && (v_rules[i].direction == IPFI_OUTPUT_POST))
          printf(TR("OUTPUT POSTROUTING SNAT. RULE N. %d"),
                 v_rules[i].position),
              PTAB;
        else if ((v_rules[i].masquerade) &&
                 (v_rules[i].direction == IPFI_OUTPUT_POST))
      ...
      if (v_rules[i].nflags.dst_addr) {
        ...
        if ((v_rules[i].parmean.damean == SINGLE) &
          ...
          printf("- %s | ", inet_ntoa(addr));
                   ...
                   (v_rules[i].nflags.dst_addr == MYADDR))
          PUND, printf(TR("IPDST: NOT MY ADDR")), printf(CLR " ");
        else if ((v_rules[i].parmean.damean == SINGLE) &&
                 (v_rules[i].nflags.dst_addr == MYADDR))
      ...
      if (v_rules[i].nflags.src_port) {
        if (v_rules[i].parmean.spmean == SINGLE)
          ...
          printf(TR("SPORT NOT: \e[4m%d\e[0m, "),
                 ntohs(v_rules[i].tp.sport[0]));
          for (j = 1; j < MAXMULTILEN && v_rules[i].tp.sport[j] != 0; j++)
            printf("\e[4m%d\e[0m, ", ntohs(v_rules[i].tp.sport[j]));
        }
      }
      if (v_rules[i].nflags.dst_port) {
        if (v_rules[i].parmean.dpmean == SINGLE)
          ...
          printf(TR("DPORT NOT: \e[4m%d\e[0m, "),
                 ntohs(v_rules[i].tp.dport[0]));
          for (j = 1; j < MAXMULTILEN && v_rules[i].tp.dport[j] != 0; j++)
            printf("\e[4m%d\e[0m, ", ntohs(v_rules[i].tp.dport[j]));
          printf("\b\b | "); /* remove last space and comma */
        }
      ...
      if (v_rules[i].nflags.indev)
        printf("[" GREEN),
            printf(TR("in device: %s"), v_rules[i].devpar.in_devname),
            printf(CLR "] ");
      if (v_rules[i].nflags.outdev)
        printf("[" GREEN),
            printf(TR("out device: %s"), v_rules[i].devpar.out_devname),
            printf(CLR "] ");
      if (v_rules[i].pkmangle.mss.enabled) {
        PNL;
        if (v_rules[i].pkmangle.mss.option == MSS_VALUE)
          printf("-- " GREEN),
              printf(TR("tcp mss: %d"), v_rules[i].pkmangle.mss.mss),
              printf(CLR " --");
        else if (v_rules[i].pkmangle.mss.option == ADJUST_MSS_TO_PMTU)
          printf("-- " GREEN), printf(TR("tcp MSS_VALUE adjust to path mtu")),
              printf(CLR " --");
      }
...
void print_menu(short filter_enabled, short resolv_services) {
  ...
  if (send_command_to_kernel(&askforloguser) < 0)
    printf(RED
           "Error sending to kernel the request for loguser enabled!" NL NL);
  ...
  if (filter_enabled) {
    printf(GRAY "K." CLR);
    printf(TR("      PRINT THE FILTER. \t"));
    printf("|");
    printf(" ["), PVIO, printf(TR("FILTER ON")), PCL, printf("]"), PNL;
  } else {
    printf(GRAY "CTRL+R." CLR);
    printf(TR(" RELOAD RULES.    "));
    printf("\t|\n");
  }

  printf("| " GRAY "I." CLR);
  printf(TR("  INSERT A RULE.         "));
  printf("\t| " GRAY "CANC." CLR);
  printf(TR("   DELETE A RULE.        "));
  printf("|");
  ...
  printf("| " GRAY "F5." CLR);
  printf(TR(" PRINT STATE TABLE.        "));
  printf("| " GRAY "CTRL+B" CLR);

  if (!uops.dns_resolver)
    PGRAY;

  printf(TR("  EDIT BLOCKED SITES. \t| "));
  PCL;

  if (getuid() == 0)
    printf("[" GREEN "%d" CLR "|" RED "%d" CLR "|" YELLOW "%d" CLR "]\n",
           acc_rules_num, den_rules_num, transl_rules_num);
  else
    printf("[" GREEN "%d" CLR "|" RED "%d" CLR "|" GRAY "*" CLR "]\n",
           acc_rules_num, den_rules_num);

  if (askforloguser.anumber == 0)
    printf("| " GRAY "V." CLR), printf(TR("  PRINT PACKETS FLOWING.    ")),
        printf("| ");
  else
    printf("| " GRAY "S." CLR), printf(TR("  SILENT MODALITY.          ")),
        printf("| ");

  if (resolv_services)
    printf(GRAY "B." CLR), printf(TR("      PORT RESOLUTION OFF.  ")),
        printf("| ");
  else
    printf(GRAY "U." CLR), printf(TR("      ENABLE PORT RESOLUT.  ")),
        printf("| ");
  ...
  if (opts.nat)
    ...
    printf("[" RED "NAT" CLR "]\n");
  printf("| " GRAY "F7." CLR);
  printf(TR(" KERNEL STATISTICS      "));
  printf("\t| " GRAY "L." CLR);
  printf(TR("      LOCAL STATISTICS."));
  printf("\t| ");
  ...
  printf("| " GRAY "C." CLR);
  printf(TR("  PRINT CONFIGURATION INFO."));
  printf("\t| " GRAY "ESC" CLR "/" GRAY "Q." CLR);
  printf(TR("  QUIT."));
  printf("\t\t\t| ");
  if (opts.masquerade)
  ...
  do {
    ...
    case CTRL_F:
      if ((counter = flush_request(nh_control, FLUSH_RULES)) < 0)
        PRED,
            printf(
                TR("print_request(): ERROR SENDING FLUSH COMMAND TO KERNEL!")),
            PNL;
      else
    ...
    case 'I':
      ...
      PNL;
      PGRAY;
      printf(TR("Type \"x\" to exit this menu without modifying rules."));
      PNL;
      if (get_new_rule(&r) < 0) {
        printf(TR("No rule added..."));
        PNL;
        goto _break;
      }
      PNL;
      PNL;
      PVIO;
      printf(TR("New rule to be added:"));
      PNL;
      PNL;
      print_rules(&r, 1, NULL);
      PNL, PVIO;
      printf(TR("Do you want to add the new rule [y | n]? "));
      PNL;
      scelta = char_translation(g_getchar());
      if ((scelta != 'y') & (scelta != 's')) {
        PNL, PVIO;
        printf(TR("New rule has not been added."));
        PNL;
        PNL;
        break;
      }
      ...
      if ((rules_updated =
               update_kernel_rules(r.nflags.policy, RELOAD_VECTOR)) < 0)
    ...
    case CTRL_B:
      ...
      if (uops.dns_resolver) {
        ...
        PNL, PRED,
            printf(TR("The refresher is not active! Start IPFIREwall with the "
                      "-dns N option,"));
        printf("\n"),
            printf(TR("where N is the refresh interval, in seconds.")), PNL,
            PNL;
      }
      prompt_return_to_menu();
    ...
    case CTRL_T:
      PNL;
      printf(TR("Saving rules..."));
      PTAB;
      if ((ret = save_rules()) < 0)
        PRED, printf(TR("FAILED")), PNL;
      else
        printf(TR("done")), PCL, printf(TR(". Saved %d rules."), ret), PNL;
      break;
    ...
    case 'w':
      if (uops.dns_resolver) {
        ...
        stop_printing();
        PNL, PRED,
            printf("The refresher is not active! Start IPFIREwall with the "
                   "-dns N option.");
        PNL, PNL;
        prompt_return_to_menu();
      }
    ...
    case 'v':
    case 'V':
      PGRN, printf(TR("Verbose modality enabled. "));
      PNL;
      printf(TR("Press "));
      printf(UNDERL GREEN "S" CLR);
    ...
    case 's':
    case 'S':
      PNL, PGRN, printf(TR("Silent modality enabled."));
      PCL;
      printf(TR("Press "));
      printf(UNDERL GREEN "v" CLR);
    ...
    case '-':
      PNL, PGRN;
      printf(
          TR("Silent modality with kernel/user communication still enabled.")),
          PNL;
    ...
    case '+':
      ...
      printf(
          TR("Use 's' and 'v' to toggle between silent and verbose modality")),
          PNL;
      printf(TR("by stopping and re-enabling kernel/user communication (more "
                "efficient!)")),
          PNL;
    ...
    case 'c':
      ...
      printf(
          ...
          "- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -" NL);
      printf(TR("User "));
      printf(GREEN UNDERL "%s" CLR "." NL NL, username);
    ...
    case 'b':
      PNL;
      printf(TR("- Disabling port resolution."));
      PNL;
    ...
    case 'u':
      PNL;
      printf(TR("- Enabling port resolution."));
      PNL;
    ...
    case '7':
      ...
      printf(
          NL BLUE
          "- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -" NL);
      PNL, printf(TR("- Requesting "));
      printf(UNDERL "kernel" CLR);
      printf(TR(" firewall statistics...")), PNL;
      memset(&firestats, 0, sizeof(firestats));
      if (request_kstats() < 0)
        PRED,
            printf(TR("Failed to send statistics request to kernel firewall!")),
            PNL, PNL;
      if (receive_kstats(&firestats) < 0)
        PRED, printf(TR("Failed to read statistics from kernel firewall!")),
            PNL, PNL;
    ...
    case 'M':
      send_command_to_listener("quiet");
      PNL;
      PUND, PYEL, PTAB, PTAB, PTAB;
      printf(TR("MAILER OPTIONS")), PNL, PNL;
      init_mailopts(&mailopts);
      if (get_mail_options(uops.mailer_options_filename, &mailopts) < 0)
...
get_in_device: /* device */
  if ((r->direction == IPFI_INPUT) || (r->direction == IPFI_INPUT_PRE) ||
      (r->direction == IPFI_FWD)) {
    printf(TR("SPECIFY THE "));
    PGRN, PUND;
    printf(TR("INPUT DEVICE"));
    printf(CLR);
...
get_out_device:
  if ((r->direction == IPFI_OUTPUT) || (r->direction == IPFI_OUTPUT_POST) ||
      (r->direction == IPFI_FWD)) {
    printf(TR("SPECIFY THE "));
    PGRN, PUND;
    printf(TR("OUTPUT DEVICE"));
    printf(CLR);
    printf(TR(" RELATED TO THE RULE: "));
...
ip_layer:
  PNL;
  printf(TR("ARE YOU INTERESTED IN SPECIFYING OTHER "));
  PUND;
  printf("IP LAYER");
  PNL;
  ...
  if ((r->ip.protocol == IPPROTO_TCP) || (r->ip.protocol == IPPROTO_UDP) ||
      (r->nflags.proto == 0)) {
    PNL, PNL, PGRN;
...
get_xxx_params: /* tcp specific parameters */
  if (r->ip.protocol == IPPROTO_TCP) {
    PNL, printf(TR("ARE YOU INTERESTED IN SPECIFYING OTHER ")), PUND,
        ...
        printf(
            TR("SPECIFIC PARAMETERS (FLAGS: SYN, ACK, RST ... ) [y | n]? [n]"));
    scelta = char_translation(g_getchar());
    if ((scelta == 's') || (scelta == 'y')) {
    ...
    PNL, printf(TR("ARE YOU INTERESTED IN CHANGING MSS_VALUE OF TCP "
                   "CONNECTIONS [y | n]? [n]"));
    scelta = char_translation(g_getchar());
    if ((scelta == 's') || (scelta == 'y')) {
  ...
  /* source nat */
get_nat:
  if ((r->nflags.policy == TRANSLATION) && (r->snat) && (r->nat)) {
  ...
  /* masquerade */
  else if ((r->nflags.policy == TRANSLATION) && (r->masquerade)) {
    if (get_masquerade_parameters(r) < 0)
      goto get_nat;
  }
  if (r->nflags.policy == ACCEPT) /* stateful rules only for permission ones. */
  {
    printf(NL), printf(TR("DO YOU WANT THIS RULE TO BE STATEFUL [y|n]? "));
...
int set_kinda_rule(const int scelta, ipfire_rule *r) {
  ...
  switch (scelta) {
  case 'p':
    PGRN, printf(TR("PERMISSION RULE:"));
    PNL;
    r->nflags.policy = ACCEPT;
    break;
...
int get_direction(ipfire_rule *r) {
  ...
  if ((r->nflags.policy == ACCEPT) | (r->nflags.policy == DENIAL)) {
  ...
  } else if ((r->nflags.policy == TRANSLATION) && (r->nat) && (!r->snat)) {
    if (!strncmp(direction, "pre", 3))
      ...
      printf(UNDERL "pre-routing" CLR DRED);
      printf(TR(" and"));
      printf(UNDERL "output" CLR DRED);
      printf(TR(" directions only."));
      PNL;
      return -1;
    }
  } else if ((r->nflags.policy == TRANSLATION) &&
             (((r->nat) && (r->snat)) || (r->masquerade))) {
    PNL;
...
int get_in_protocol(ipfire_rule *r) {
  ...
  if ((strncmp(proto, "tcp", 3) == 0) || ((strncmp(proto, "TCP", 3) == 0)))
    ...
    r->ip.protocol = IPPROTO_UDP;
           ...
           ((strncmp(proto, "ICMP", 4) == 0)))
    r->ip.protocol = IPPROTO_ICMP;
  else if ((strncmp(proto, "igmp", 4) == 0) ||
           ((strncmp(proto, "IGMP", 4) == 0)))
    r->ip.protocol = IPPROTO_IGMP;
  else {
    PRED, printf(TR("Protocol \"%s\" not valid!"), proto), PNL;
    return -1;
  }
  r->nflags.proto = 1;
  return 0;
  ...
  if (addr[0] == '!') /* address different from */
  {
    ...
    if (is_interval(addr)) {
      ...
      if ((r->nflags.src_addr) &
        ...
        printf(TR("IP SRC NOT IN INTERVAL %s"), inet_ntoa(ina));
        printf(" - %s\n", inet_ntoa(ina2));
      } else if ((r->nflags.dst_addr) &
                 (r->parmean.damean == INTERVAL_DIFFERENT_FROM) &
                 (direction == DEST)) {
        ina.s_addr = r->ip.ipdst[0];
        ina2.s_addr = r->ip.ipdst[1];
        printf(TR("IP DST NOT IN INTERVAL %s"), inet_ntoa(ina));
        printf(" - %s\n", inet_ntoa(ina2));
      }
    } else {
 ...
 * -1 otherwise */
int check_tos(u8 tos) { return 0; }

/* checks if total length value is permitted. Returns 0 on success,
 * -1 otherwise */
int check_totlen(u16 total_length) { return 0; }
...
int get_tcp_specific_parameters(ipfire_rule *r) {
  ...
  printf(NL);
  /* SYN */
  PNL, PGRN, printf(TR("SYN FLAG MUST BE: ACTIVE [a or 1],  OFF [o or 0],")),
      PNL, printf(TR("DON'T CARE [other]: "));
  scelta = char_translation(g_getchar());
  switch (scelta) {
  case 'a':
  case '1':
  ...
  /* ACK */
  PNL, PGRN, printf(TR("ACK FLAG MUST BE: ACTIVE [a or 1],  OFF [o or 0],"));
  PNL;
  ...
  /* URG */
  PNL, PGRN, printf(TR("URG FLAG MUST BE: ACTIVE [a or 1],  OFF [o or 0],"));
  PNL;
  printf(TR("DON'T CARE [other], DONE, EXIT [e]: "));
  scelta = char_translation(g_getchar());
  switch (scelta) {
  ...
  /* PSH */
  PNL, PGRN, printf(TR("PUSH FLAG MUST BE: ACTIVE [a or 1],  OFF [o or 0],"));
  PNL;
  printf(TR("DON'T CARE [other], DONE, EXIT [e]: "));
  scelta = char_translation(g_getchar());
  switch (scelta) {
  ...
  /* RST */
  PNL, PGRN, printf(TR("RESET FLAG MUST BE: ACTIVE [a or 1],  OFF [o or 0],"));
  PNL;
  printf(TR("DON'T CARE [other], DONE, EXIT [e]: "));
  scelta = char_translation(g_getchar());
  switch (scelta) {
  ...
  /* FIN */
  PNL, PGRN, printf(TR("FIN FLAG MUST BE: ACTIVE [a or 1],  OFF [o or 0],"));
  PNL;
  printf(TR("DON'T CARE [other], DONE, EXIT [e]: "));
  scelta = char_translation(g_getchar());
  switch (scelta) {
...
int get_mss_parameters(ipfire_rule *r) {
  ...
  int mss;
  int ret;
  PNL, printf(TR("DO YOU WANT TO SPECIFY A VALUE [v] OR LET THE FIREWALL "
                 "AUTOMATICALLY ADJUST IT TO PMTU [a] ? [a]"));
  scelta = char_translation(g_getchar());
  if (scelta == 'v') {
    printf(TR("TYPE AN INTEGER VALUE FOR THE Maximum Segment Size (max 1500 - "
              "40 = 1460): "));
    ret = scanf("%d", &mss);
    while (getchar() != '\n')
      ;
    if (ret <= 0) {
      printf(TR("INVALID INTEGER"));
      PNL;
      return -1;
    }
    if (mss > 1460 || mss <= 0) {
      printf(TR(
          "MSS_VALUE MUST BE GREATER THAN 0 AND LESS THAN OR EQUAL TO 1460"));
      return -1;
    } else {
...
void quiet_modality(int quiet) {
  if (!quiet) {
    printf(VIOLET UNDERL);
    printf(TR("WARNING"));
    PCL;
    printf(TR(": printing packets has been disabled for now:"));
    PNL;
    printf(TR("         high network traffic/low resources on machine"));
    PNL;
    printf(TR("         caused packet loss from kernel/user communication."));
    PNL;
    printf(TR("         Press "));
    printf(GREEN "'v'" CLR);
    printf(TR("key to re-enable verbose printing."));
    PNL;
    quiet = 1;
  }
}
...
int print_my_rules(void) {
  ...
  if ((getuid() != 0) && (!opts.user_allowed))
    PVIO, printf(TR(" ! RULES SHOULD NOT BE LOADED:")),
        printf(
            TR("   USER DOES NOT SEEM TO BE ALLOWED TO INSERT HIS OWN RULES.")),
        PNL, printf(TR("   CHECK WITH F3 AND CONTACT ADMINISTRATOR.")), PNL;
...
void print_stats(struct netlink_stats *ns) {
  ...
  PUND;
  printf(TR("USERSPACE FIREWALL"));
  PCL;
  PRED;
  printf(TR(" POINT OF VIEW"));
  PCL;
  PNL;
  PNL;
...
void print_kstats(const struct kernel_stats *ks) {
  ...
  total = ks->in_rcv + ks->out_rcv + ks->fwd_rcv + ks->pre_rcv + ks->post_rcv;

  /* update percentage */
  if (total != 0)
    lost_percent = (float)((((float)ks->total_lost / (float)total)) * 100);
  else
    lost_percent = 0;

  PNL, PRED, PTAB, PTAB;
  printf(TR("* STATISTICS FROM "));
  printf(UNDERL "KERNEL" CLR RED);
  printf(TR(" POINT OF VIEW *")), PNL, PNL;

  printf(TR("Kernel module has been loaded since %s"),
         ctime(&ks->kmod_load_time));

  /* Calculate current time and the number of seconds elapsed between kernel
   * module loading time and now. Then calculate corresponding hours, minutes
   * and seconds.
   */
  time(&current_time);
  ...
  printf(TR("[%.2f/day %.2f/h %.2f/min %.2f/s]"), ks->in_rcv / days,
         ks->in_rcv / hours, ks->in_rcv / mins, ks->in_rcv / secs_difftime);
  PNL;

  printf("- OUTPUT: \t%llu\t", ks->out_rcv);
  printf(TR("[%.2f/day %.2f/h %.2f/min %.2f/s]"), ks->out_rcv / days,
         ks->out_rcv / hours, ks->out_rcv / mins, ks->out_rcv / secs_difftime);
  PNL;

  printf("- FORWARD: \t%llu\t", ks->fwd_rcv);
  printf(TR("[%.2f/day %.2f/h %.2f/min %.2f/s]"), ks->fwd_rcv / days,
         ks->fwd_rcv / hours, ks->fwd_rcv / mins, ks->fwd_rcv / secs_difftime);
  PNL;

  printf("- PRE ROUTING: \t%llu\t", ks->pre_rcv);
  printf(TR("[%.2f/day %.2f/h %.2f/min %.2f/s]"), ks->pre_rcv / days,
         ks->pre_rcv / hours, ks->pre_rcv / mins, ks->pre_rcv / secs_difftime);
  PNL;

  printf("- POST ROUTING: %llu\t", ks->post_rcv);
  printf(TR("[%.2f/day %.2f/h %.2f/min %.2f/s]"), ks->post_rcv / days,
         ks->post_rcv / hours, ks->post_rcv / mins,
         ks->post_rcv / secs_difftime);
  PNL;
  ...
  if (ks->in_rcv != 0) {
    ...
    PCL;
    printf(TR("INPUT PACKETS"));
    PRED, printf(TR(" DROPPED"));
    PCL;
    printf(": %llu [%.1f%%]." NL, ks->in_drop, perc_in_drop);

    PCL;
    printf(TR("INPUT PACKETS"));
    PGRN, printf(TR(" ACCEPTED"));
    PCL;
    printf(": %llu [%.1f%%]." NL, ks->in_rcv - ks->in_drop,
           (float)100 - perc_in_drop);

    if (ks->in_drop_impl > 0)
      printf(TR("INPUT PACKETS DROPPED ")), PVIO, printf(TR("IMPLICITLY")), PNL,
          printf(TR("[WITH DEFAULT POLICY]: %llu [%.1f%%]."), ks->in_drop_impl,
                 perc_in_drop_impl),
          PNL, PNL;

    if (ks->in_acc_impl > 0)
      printf(TR("INPUT PACKETS ACCEPTED ")), PVIO, printf(TR("IMPLICITLY")),
          PNL,
  ...
  if (ks->out_rcv != 0) {
    ...
    PGRN, printf(TR(" ACCEPTED"));
    PCL;
    printf(": %llu [%.1f%%]." NL, ks->out_rcv - ks->out_drop,
           (float)100 - perc_out_drop);

    if (ks->out_drop_impl > 0)
      printf(TR("OUTPUT PACKETS DROPPED ")), PVIO, printf(TR("IMPLICITLY")),
          PNL,
    ...
    if (ks->out_acc_impl > 0)
      printf(TR("OUTPUT PACKETS ACCEPTED ")), PVIO, printf(TR("IMPLICITLY")),
          PNL,
  ...
  if (ks->fwd_rcv != 0) {
    ...
    PCL;
    printf(TR("FORWARD PACKETS"));
    PRED, printf(TR(" DROPPED"));
    PCL;
    printf(": %llu [%.1f%%]." NL, ks->fwd_drop, perc_fwd_drop);

    PCL;
    printf(TR("FORWARD PACKETS"));
    PGRN, printf(TR(" ACCEPTED"));
    PCL;
    printf(": %llu [%.1f%%]." NL, ks->fwd_rcv - ks->fwd_drop,
           (float)100 - perc_fwd_drop);

    if (ks->fwd_drop_impl > 0)
      printf(TR("FORWARD PACKETS DROPPED ")), PVIO, printf(TR("IMPLICITLY")),
          PNL,
    ...
    if (ks->fwd_acc_impl > 0)
      printf(TR("FORWARD PACKETS ACCEPTED ")), PVIO, printf(TR("IMPLICITLY")),
          PNL,
  ...
  printf(TR("- Packets ")), printf(DVIOLET UNDERL), printf(TR("not sent")), PCL,
      printf(TR(" to user firewall due to kernel/user log level: %llu."),
             ks->not_sent);
  PNL;

  printf(TR("- Packets ")), PUND, PRED, printf(TR("lost")), PCL,
      printf(TR(" while sending towards userspace: %llu"), ks->total_lost), PNL;
  ...
  printf(TR("  over %llu arrived (bad checksum). [%.2f %%]"), ks->pre_rcv,
         (float)ks->bad_checksum_in / (float)ks->pre_rcv * (float)100),
      PNL;
...
int prompt_return_to_menu(void) {
  ...
  if (prog_ops.quiet == 0) {
    if (send_command_to_listener("verb") < 0) {
      PRED, printf(TR("prompt_return_to_menu(): error sending enable printing "
                      "command"));
      PNL;
      return -1;
    }
...
int check_port(int p) { return ((p < 65536) && (p > 0)); }
...
int get_nat_parameters(ipfire_rule *r, int nat_kind) {
  ...
  if (nat_kind == SOURCE_NAT) {
    ...
    PNL, PGRN, printf(TR("DESTINATION NETWORK ADDRESS TRANSLATION: ")), PNL,
        PNL;
...
int get_masquerade_parameters(ipfire_rule *r) {
  ...
  if (!r->nflags.outdev) {
    ...
    if (get_device(r, OUTDEV) < 0) {
      PNL, PNL, PRED,
          printf(TR("get_masquerade_parameters(): error getting device name!")),
          PNL;
      return -1;
    }
  }
  ...
  if (!r->nflags.outdev) {
    PRED, printf(TR("I CAN'T MASQUERADE AN UNSPECIFIED OUTPUT INTERFACE!"));
    PNL;
    return -1;
  }
  return 0;
...
int blocked_sites_management(const char *filename) {
  ...
  while (fgets(tmpname, 1024, fp) != NULL && i < nlines) {
    if (strlen(tmpname) > 1 && tmpname[0] != '#' && tmpname[0] != '\n') {
      ...
      if (names[i] == NULL) {
        PRED, printf(TR("Error allocating memory for the blocked site entry "
                        "no %d "),
                     i + 1);
        perror(""), PNL;
        free(names);
  ...
  do {
    ...
    case 'd':
      if (nlines > 0) {
        printf(TR("Insert the number corresponding to the line you want to "
                  "remove [%d-%d] [+return], "),
               1, nlines);
        ...
        if (xline < 1 || xline > nlines) {
          PRED,
              printf(TR("Number %d out of the range %d-%d!"), xline, 1, nlines);
          break;
        } else {
          ...
          if (scelta2 == 'y') {
            ...
            if (list_updated == NULL)
              PRED,
                  printf(TR("An error occurred while removing an entry, so it "
                            "won't be removed!")),
                  PNL;
            else {
              /* Update the copy */
              ...
              printf(TR("Confirm by pressing the 's' key when finished")), PNL,
                  PNL;
    ...
    case 'S':
      ...
      fprintf(
          fp,
          "# Put here Internet addresses you want to be blocked.\n"
          ...
          "#\n"
          "#\n"
          "# File is read by IPFIREWALL 'resolver' if ipfire is started with "
          "option '-dns N',\n"
          "# where N is the interval in seconds after which names are "
          "refreshed.\n"
          "#\n"
          "# (C) Giacomo S.\n"
          "# june 2005 - march 2007 delleceste@gmail.com\n"
          "#\n"
          "# Since version 0.98.4 this file can be managed by the main menu "
          "interface.\n"
          "# This is a recommended usage unless you have to paste a long "
          "list "
          "in this\n"
          "# file.\n"
          "# Have a nice time :)\n"
    ...
    case 'A':
      ...
      if (fgets(tmpname, 1024, stdin) != NULL) {
        ...
        if (scelta2 == 'y') {
          ...
          /* Add the new rule */
          tmpnames[nlines] =
              (char *)malloc(sizeof(char) * (strlen(tmpname) + 1));
          ...
          names = tmpnames;
          PNL, PGRN, printf(TR("OK"));
          PNL;
          nlines++; /* ...which has one more element! */
          modified = 1;
        } else
          PNL, printf(TR("Operation canceled!"));
        PNL;
      }
    }
...
char **remove_line(char **list, int nlines, int cancline) {
  ...
  if (copy == NULL) {
    PRED,
        printf(TR("Error allocating memory for the copy of the blocked list "));
    perror(""), PNL;
    return NULL;
  }
  ...
  for (i = 0; i < nlines; i++) {
    ...
    if (copy[j] == NULL) {
      PRED, printf(TR(
                "Error allocating memory for the copy of the blocked list "));
      perror(""), PNL;
      return NULL;
    }
