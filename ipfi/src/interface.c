...
#include "includes/rule_cache.h" /* Kernel rule cache for logging rulenames */
...
int main(int argc, char *argv[]) {
  ...
  if (geteuid() == 0) {
    ...
    if ((ret = load_module()) < 0)
      printf("Loading module \"%s\"...\t[" RED, MODULENAME),
          printf("%s", TR("FAILED")), printf(CLR "]" NL);
    //       else if(ret > 0)
    // 	printf("module \"%s\"...\t[" GREEN, MODULENAME), printf(TR("ALREADY
    // LOADED") ), printf(CLR "]" NL);
    //       else
  ...
  if (parse_cmdline(&prog_ops, &uops, &opts, argc, argv,
                    &rules_filenames_changed) < 0) {
    printf(NL RED UNDERL "Errors in command line parameters." NL GREEN
                         "Try \"%s -help\"" NL,
           argv[0]);
    exit(EXIT_FAILURE);
  }
  ...
  num_lang_strings =
      allocate_translation_strings(uops.language_filename, uops.rc);
  ...
  if ((!uops.rc) && rules_filenames_changed != 0) {
    ...
    PTAB;
    printf("\"%s\".", uops.permission_filename);
    printf(NL);
    ...
    if (uops.dns_resolver)
      printf(TR("Blocked sites rules:")), PTAB,
          printf("\"%s\".", uops.blacksites_filename), printf(NL NL);
    else
      printf(NL);
  ...
  //   printf(TR("There are %d denial, %d permission"), den_rules_num,
  //   acc_rules_num);
  ...
  if (geteuid() == 0) {
    ...
    //       printf(TR(" and %d translation rules."), transl_rules_num),
    //       printf(NL);
  }
  //   else
  ...
  if (!nh_control || (hellocode = hello_handshake(&hellocmd)) < 0) {
    PNL, PRED, printf(TR("Error sending hello to firewall.")), PNL, PNL,
        printf("- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - "
               "- - - -\n");
    printf(TR("Be sure that root has loaded IPFIRE module into the kernel!")),
        PNL,
        printf(TR(
            "Type \"/sbin/lsmod\" on the console and look for \"ipfi_...\"")),
        PNL,
        printf(TR("modules in the list. If you don't see them, IPFIRE is not "
                  "loaded")),
        PNL, printf(TR("into the kernel and so this error happened.")),
        printf("\n- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - "
               "- - - - -\n");
    PNL;
    exit(EXIT_FAILURE);
    ...
    if (hellocode == IPFIRE_BUSY)
      PNL, PRED, printf(TR("ipfire is busy!")), PNL,
          printf("- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - "
                 "- - - - -\n"),
          printf(TR("Another instance of an userspace interface seems to be "
                    "running, ")),
          printf(TR("with pid %d!"), hellocmd.anumber), PNL,
          printf(TR("That instance should be terminated, e.g. with the command "
                    "\"kill %d\"."),
                 hellocmd.anumber),
          PNL,
          printf("- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - "
      ...
      /* Fix: cast were needed if command was not fully defined,
       * but now it is. We keep it standard. */
  ...
  if (send_command_to_kernel(&opts) < 0) {
    PNL, PRED, PUND;
    printf(TR("ERROR SENDING CONFIGURATION OPTIONS TO KERNEL!")), PNL, PCL,
        PRED, printf(TR("BE SURE TO HAVE LOADED IPFI KERNEL MODULE!")), PNL;
    if (geteuid() != 0)
  ...
  if (read_command_from_kernel(&opts) < 0) {
    PRED, printf(TR("main() error receiving message from kernel firewall")),
        PNL;
    exit(EXIT_FAILURE);
  } else {
  ...
  if (send_rules_to_kernel(accept_rules, acc_rules_num) <
      0) { /* eventually free pointer denial_rules... */
    PRED, printf(TR("Error sending permission rules to kernel space!")), PNL;
    exit(EXIT_FAILURE);
  }
  if ((geteuid() == 0) && (translation_rules != NULL)) {
    if (!uops.rc && 0)
      printf(TR("Sending ")), PUND, printf(TR("translation")), PCL,
          printf(TR(" rules...")), PNL;
    if (send_rules_to_kernel(translation_rules, transl_rules_num) < 0)
      PRED, printf(TR("Error sending translation rules to kernel")), PNL;
  } else if ((geteuid() != 0) && (translation_rules != NULL))
    PVIO,
        printf(TR("Warning: you are not root and cannot send translation rules "
                  "to kernel!")),
        PNL;
  ...
  if ((uops.dns_resolver) && (!uops.flush)) {
    resolver_pid =
        start_resolver(uops.dns_refresh, uops.justload, semid_lockfile);
  }

  if (uops.justload) /* if '-rc' is given, both 'justload' and 'rc' are set. */
  {
    // printf("- Option \"justload\" [or \"rc\"] is set: exiting leaving rules
    // in firewall." si		NL);
  ...
  /* ========================================================================
   * FIX FOR RULE NAME LOOKUP IN PACKET LOGS:
   *
   * When root loads firewall rules (via rc.ipfire start) and a normal user
   * runs the logging interface, the logger's userspace process doesn't have
   * the rules in its local memory from the normal user's config files.
   *
   * Solution: We fetch ALL currently active rules from the kernel space
   * (which contains the rules loaded by any user, including root) and
   * cache them locally. Since we do this BEFORE forking, both the main
   * interaction process and the listener process will share this cache.
   * ======================================================================== */
  if (init_rule_cache_from_kernel(nh_control) < 0) {
    PRED, printf(TR("Warning: Failed to initialize rule cache from kernel.")),
        PNL;
    printf(TR("Rule names may not display in packet logs.")), PNL;
  }
...
int listener(void) {
  ...
  if (uops.resolv_services) {
    ...
    if (svent == NULL) {
      PRED,
          printf(TR("Failed to allocate space for port services. Disabling.")),
          PNL, PNL;
  ...
  if (!nh_data ||
      (send_to_kernel((void *)&lmess, nh_data, LISTENER_DATA) < 0)) {
  ...
  while (1) {
    if ((bytes_read = read_from_kern(nh_data, (unsigned char *)&mes_from_kern,
      ...
      case IPFI_OUTPUT:
        nlstats.out_rcv++;
        nlstats.last_out_rcv++;
        break;
      case IPFI_FWD:
        nlstats.fwd_rcv++;
        nlstats.last_fwd_rcv++;
        break;
      case IPFI_INPUT_PRE:
        nlstats.pre_rcv++;
        nlstats.last_pre_rcv++;
        break;
      case IPFI_OUTPUT_POST:
        nlstats.post_rcv++;
        nlstats.last_post_rcv++;
        break;
      }
      if (quiet == 0)
  ...
  if (signum == SIGUSR1) {
    ...
    if (read(pipefd[0], buf, MAXPIPESIZEMESS) < 0)
      ...
      if (process_cmd_from_pipe(buf) < 0)
        PRED,
            printf(TR("sig1_handler(): error processing command \"%s\""), buf),
            PNL;
    }
  }
}
  ...
  if (!strcmp(com, "quiet"))
    ...
    if ((uops.resolv_services == 0) && (svent == NULL)) {
      ...
      if (svent == NULL) {
        PRED,
            printf(
                TR("Failed to allocate space for port services. Disabling.")),
            PNL;
        uops.resolv_services = 0;
      }
      printf("[" GREEN "OK" CLR ".]" NL);
      ...
      PNL, printf(TR("Service name resolution seems to be already active!")),
          PNL, PNL;
    ...
    if ((uops.resolv_services) && (svent != NULL)) {
      ...
      PNL, printf(TR("Service name resolution seems to be already disabled!")),
          PNL, PNL;
    ...
    if (filter != NULL) {
      PVIO,
          printf(TR(
              "Warning: deleting the old filter and creating a new one...\t")),
          PCL;
...
void dnsres_handler(int signum) {
  ...
  if (signum == SIGUSR2) {
    PYEL,
        printf(TR(
            "Updating kernel denial rules after updating blocked sites list.")),
        PNL;
    if ((rules_updated = update_kernel_rules(DENIAL, RELOAD_FILE)) < 0)
      PRED, printf(TR("Error updating blacklist rules!")), PNL;
    else
      PGRN, printf(TR("Blacklist rules updated [%d rules]!"), rules_updated),
          PNL;
  }
}
...
void mailer_read_handler(int signum) {
  ...
  if (signum == SIGUSR1) {
    ...
    if (request_kstats() < 0)
      PRED, printf(TR("Failed to send statistics request to kernel firewall!")),
          PNL;
    if (receive_kstats(&firestats) < 0)
      PRED, printf(TR("Failed to read statistics from kernel firewall!")), PNL,
          PNL;
    printf(TR("WRITING TO PIPE")), PNL;
    if (write(mailpipefd[1], (void *)&firestats, sizeof(firestats)) < 0)
      PRED,
          perror(
              TR("mailer_read_handler(): failed to write to pipe for mailer!")),
          PCL;
  }
}
...
void signal_handler(int signum) {
  ...
  free_mem:
    ...
    if ((geteuid() == 0) && (!prog_ops.noflush_on_exit)) {
      if (uops.rmmod) {
        ...
        PNL, printf(TR("Kernel module will not be unloaded: the default"));
        PNL;
        printf(TR("policy specified at module loading time will be applied."));
        PNL;
        PNL;
        printf("\e[37;41;1m" UNDERL);
        printf(TR("WARNING"));
        printf(CLR RED);
        printf(TR(": the firewall has been stopped but the kernel module "
                  "wasn't unloaded."));
        PNL;
        printf(TR("         If the firewall policy was to drop packets which "
                  "do not match any"));
        PNL;
        printf(TR("         rule, then many applications could stop working "
                  "until you remove"));
        PNL;
        printf(TR("         IPFIRE module from the kernel or restart IPFIRE "
                  "itself."));
        printf("\e[00m\n\n");
      }
    }
...
void print_signal(int signum) {
  ...
  case SIGSEGV:
    ...
    printf(TR("FATAL ERROR! Report to "));
    printf(UNDERL "delleceste@gmail.com" NL);
    printf(TR("Trying to communicate last goodbye to kernel firewall...")), PNL;
    break;
  }
}

int get_quiet_state(void) { return prog_ops.quiet; }
