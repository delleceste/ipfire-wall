...
int state_machine(const struct sk_buff *skb, int current_state, short reverse) {
   ...
   * from the other host. */
  else if ((current_state == SYN_SENT) && (syn == 1) && (ack == 1) &&
           (reverse == 1) && (!rst) && (!fin))
    state = SYN_RECV;
  ...
  else if (((current_state == SYN_RECV) || (current_state == GUESS_SYN_RECV)) &&
  ...
  else if ((current_state == ESTABLISHED) && (syn == 1) && (ack == 1) &&
           (reverse == 1) && (!rst) && (!fin))
    state = ESTABLISHED;
  /* D: FIN WAIT */
    ...
    if (syn)
      ...
      state = GUESS_CLOSING;
  } else if ((current_state == FIN_WAIT) && (ack) && (fin) && (reverse))
    state = LAST_ACK;
...
int set_state(const struct sk_buff *skb, struct state_table *entry,
              short reverse) {
  int state;
  /* Get the state */
  state = state_machine(skb, entry->state.state, reverse);
  /* Normalise guess states */
  if (state == GUESS_CLOSING)
    ...
    state = ESTABLISHED;
  /* Only write if the state actually changed — avoids dirtying
   * the cache line on every packet in the common steady-state
   * case (e.g. UDP_ESTAB → UDP_ESTAB). */
