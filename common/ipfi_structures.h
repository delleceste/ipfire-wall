 ...
 * 
 * The IPFire userspace application was historically written using the 'igmp_...' 
 * field names, but the shared 'packet_headers' union uses 'struct igmphdr'. 
 * To ensure the same source code (like src/utils.c) builds successfully for 
 * both targets without heavy #ifdef pollution in the logic files, we define 
 * a userspace version of 'struct igmphdr' here that maps to the field names 
