...
struct nat_table *add_snatted_entry(const struct sk_buff *skb,
  ...
  ipfi_entry_init(&entry->h,
                  get_timeout_by_state(entry->protocol, entry->state),
                  handle_nat_entry_timeout);

  refcount_set(&entry->h.refcnt, 1);

  {
    unsigned int bkt = entry->bkts[NAT_IDX_ORIG];

    spin_lock_bh(&nat_bucket_locks[NAT_SNAT][NAT_IDX_ORIG][bkt]);
  ...
  /* Add POSTNAT index (C -> B): hash(new_addr, new_port, old_daddr, old_dport)
   */
  {
    u32 postnat_key =
        get_nat_tuple_hash(entry->new_addr, entry->new_port, entry->old_daddr,
                           entry->old_dport, entry->protocol);
    nat_add_index(entry, NAT_IDX_POSTNAT, postnat_key);
  }

  /* Add REPLY index (B -> C): hash(old_daddr, old_dport, new_addr, new_port) */
  {
    u32 reply_key =
        get_nat_tuple_hash(entry->old_daddr, entry->old_dport, entry->new_addr,
                           entry->new_port, entry->protocol);
    nat_add_index(entry, NAT_IDX_REPLY, reply_key);
  }
...
int de_snat_table_match(struct nat_table *snt, struct sk_buff *skb) {
  ...
  if (snt->protocol != IPPROTO_TCP && snt->protocol != IPPROTO_UDP) {
    ...
    if ((nquad.saddr == snt->old_daddr) && (nquad.sport == snt->old_dport) &&
        (nquad.daddr == snt->new_addr) && (nquad.dport == snt->new_port))
      return 1;
  }
  return -1;
}

/* Source de-NAT (PREROUTING).
 * Uses lookup_nat_idx which handles RCU internally.
 */
int pre_de_snat(struct sk_buff *skb, const ipfi_flow *flow,
                struct response *resp, struct info_flags *flags) {
  struct nat_table *sntmp = lookup_nat_idx(NAT_SNAT, NAT_IDX_REPLY, skb);
...
/* Dynamic SNAT/Masquerade (POSTROUTING).
 * Uses lookup_nat_idx which handles RCU internally.
 */
int post_snat_dynamic(struct sk_buff *skb, const ipfi_flow *flow,
  ...
  if (dntmp) {
    if (snat_dynamic_table_match(dntmp, skb) > 0) {
      ...
      int ret = snat_dynamic_translate(skb, dntmp, flow);

      /* Update/Add Reply index with the new Masquerade address.
       * Always call nat_add_index; it handles key updates (moves) if needed.
       */
      {
        u32 reply_key = get_nat_tuple_hash(dntmp->new_addr, dntmp->new_port,
                                           dntmp->our_ifaddr, dntmp->old_sport,
                                           dntmp->protocol);
...
int snat_dynamic_translate(struct sk_buff *skb, struct nat_table *dnt,
  ...
  /* Warning: we pass skb to get_ifaddr since get_ifaddr doesn't reallocate it
   */
  dnt->our_ifaddr = get_ifaddr(skb, flow->out);
