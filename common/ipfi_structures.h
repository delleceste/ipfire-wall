...
// #warning ("\033[1;31mKERNEL MODE includes linux/tcp.h, user includes
// netinet/tcp.h\033[0m")
#ifdef __KERNEL__
...
/*
 * [COMPATIBILITY LAYER: IGMP]
 * The Linux kernel defines 'struct igmphdr' (in <linux/igmp.h>) with fields:
 *      type, code, csum, group.
 * Userspace headers (Glibc <netinet/igmp.h>) provide 'struct igmp' with fields:
 *      igmp_type, igmp_code, igmp_cksum, igmp_group.
 *
 * The IPFire userspace application was historically written using the
 * 'igmp_...' field names, but the shared 'packet_headers' union uses 'struct
 * igmphdr'. To ensure the same source code (like src/utils.c) builds
 * successfully for both targets without heavy #ifdef pollution in the logic
 * files, we define a userspace version of 'struct igmphdr' here that maps to
 * the field names expected by the application.
 */
struct igmphdr {
  uint8_t igmp_type;
  uint8_t igmp_code;
...
struct info_flags {
  __u16 direction : 3,      /* in, out or forward */
      ...
      snat : 1, badsum : 1, /* bad checksum */
      external : 1,         /* packet is arriving on external interface (nat) */
      ftp : 2,
      exit
      : 1, /* notifies that the userspace thread (on the GUI only) can finish */
      nat_max_entries
      : 1, /* maximum number of entries reached in the kernel tables */
      snat_max_entries
      : 1, /* in these cases packet_id contains the number representing */
      state_max_entries : 1, /* the limit reached */
      _free : 3; /* in conjunction with `state' flag above, indicates a packet
                    reverse-matched in a stateful connection */
};
...
typedef struct { /* see linux/skbuff.h */
  ...
  // Initialization of struct members must be done at instantiation, not in
  // definition. memset(&response, 0, sizeof(response)); response.verdict =
  // IPFI_IMPLICIT;
} ipfire_info_t;
...
typedef struct {
  ...
  __u8 syn : 1, fin : 1, rst : 1, psh : 1, ack : 1, urg : 1, free1 : 1,
      free2 : 1;
} transparams;
...
typedef struct {
  __u16 spmean : 3, dpmean : 3, samean : 3, damean : 3;
} meanings;
...
typedef struct {
  ...
  __u16 src_addr : 2, dst_addr : 2, /* NONE (0), ONEADDR, MYADDR */
      ...
      /* tcp/transport */
      src_port : 1, dst_port : 1, syn : 1, fin : 1, rst : 1, psh : 1, ack : 1,
      urg : 1,
...
typedef struct {
  ...
  __u16 is_rule : 1, /* is it a rule or not? */
      ...
      loglevel : 3, loguser : 3, noflush_on_exit : 1,
      user_allowed : 1, /* another option, not used for now */
      all_stateful : 1;
...
struct ipfi_counters {
  ...
  unsigned long long in_sent_touser;
  unsigned long long out_sent_touser;
  unsigned long long fwd_sent_touser;
  unsigned long long pre_sent_touser;
  unsigned long long post_sent_touser;
