...
direction:
  ...
  case IPPROTO_TCP:
    flogpack(TCP);
    snprintf(c, LOGLINELEN, "|%s|%d|%s|%d", src_address,
             ntohs(pack->packet.transport_header.tcphead.th_sport), dst_address,
  ...
  case IPPROTO_UDP:
    flogpack(UDP);
    snprintf(c, LOGLINELEN, "|%s|%d|%s|%d", src_address,
             ntohs(pack->packet.transport_header.udphead.uh_sport), dst_address,
  ...
  case IPPROTO_ICMP:
    flogpack(ICMP);
    snprintf(c, LOGLINELEN, "|%s|%s", src_address, dst_address);
    flog(c);
    /* fill 6 flags  fields with don't cares */
    flog("|0|0|0|0|0|0");
    break;
