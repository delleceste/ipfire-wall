...
static inline void nat_hold(struct nat_table *nt) { ipfi_entry_hold(&nt->h); }

static inline bool nat_hold_rcu(struct nat_table *nt) {
  return ipfi_entry_hold_rcu(&nt->h);
}

static inline void nat_put(struct nat_table *nt) { ipfi_entry_put(&nt->h); }
 ...
 * In hash mode, entries are stored in nat_hashtables and nat_counters.
