...
int iph_in_get_response(ipfire_info_t * ipfi_info, struct sk_buff* skb)
{
	...
	extern short found_in_state_table;
	...
	/* set the field response of ipfi_info in accord with engine verdict */
	ipfi_info->response = response;
	ipfi_info->state = found_in_state_table;
	...
	if ((userspace_data_pid) && (loguser_enabled) && (is_to_send(ipfi_info, &fwopts) > 0))
	{
		...
		if(skb_touser == NULL) /* shouldn't happen :r */
		  ...
		  update_kernel_stats(IPFI_FAILED_NETLINK_USPACE, ipfi_info->packet_id);
		 ...
		 */
		if(gui_notifier_enabled && ((response == 0 && ipfi_info->direction != IPFI_FWD) || (response != 0 && ipfi_info->notify)) )
		{
