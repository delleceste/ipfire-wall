...
void *extract_data(struct sk_buff *skb) {
  ...
  if (nlmsglen < sizeof(*nlh) || skblen < nlmsglen) {
    IPFI_PRINTK("IPFIRE: nlmsglen < sizeof(*nlh) || skblen < nlmsglen)  in "
                "extract_data()\n");
    return NULL;
  }
...
int process_control_received(struct sk_buff *skb) {
  ...
  if (cmd_from_user == NULL) {
    IPFI_PRINTK("IPFIRE: process_control_received(): error extracting data "
                "from socket buffer\n");
    return -1;
  }
...
void fill_firesizes_with_kernel_values(command *cmd, size_t krulesize,
                                       size_t kinfosize, size_t kcmdsize,
                                       uid_t uspace_uid) {
...
int initial_handshake(command *hello, uid_t userspace_uid) {
  ...
  kernelsizes =
      (struct firesizes *)kmalloc(sizeof(struct firesizes), GFP_KERNEL);

  if (kernelsizes == NULL) {
    IPFI_PRINTK("IPFIRE: failed to allocate memory for struct firesizes in "
                "initial_handshake()\n");
    return -1;
  }
  ...
  if (result != HELLO_OK) {
    ...
    fill_firesizes_with_kernel_values(hello, kernelsizes->rulesize,
                                      kernelsizes->infosize,
                                      kernelsizes->cmdsize, userspace_uid);
  }
...
int set_firewall_options(command *cmd, const uid_t commander) {
  if (commander == 0) {
    ...
    setup_shutd_state_lifetime = cmd->setup_shutd_state_lifetime;
    fwopts.setup_shutd_state_lifetime = cmd->setup_shutd_state_lifetime;
...
void opts_to_cmd(command *cmd) {
  ...
  cmd->setup_shutd_state_lifetime = fwopts.setup_shutd_state_lifetime;
...
void print_state_entries_memory_usage(void) {
  ...
  IPFI_PRINTK("IPFIRE: size of a source nat table is %zu bytes, so"
              ...
              (max_state_entries * sizeof(struct state_table)) / 1024);
}
...
void print_loginfo_memory_usage(unsigned long lifetime) {
  ...
  IPFI_PRINTK("max n. of entries is %u\n"
              ...
              (unsigned long)((max_loginfo_entries *
